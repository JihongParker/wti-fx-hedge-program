...
A Joint Model of Disclosure Intensity and Hedge-Ratio Choice,\\
with a Difference-in-Differences Design for Korean Listed Firms}}
...
\begin{abstract}
The three companion papers of this program take a single hedged position -- a Korean crude-oil importer's joint WTI and USD/KRW exposure -- and ask, in turn, how much of it to hedge under a budget, how to hedge it dynamically, and how IFRS~9 accounts for the choice. This paper asks what moves the choice in the first place: does mandatory ESG disclosure change how much, and how efficiently, firms hedge? I formalize the firm as jointly choosing a financial hedge ratio $h_f$, a climate-risk hedge ratio $h_c$, and a disclosure intensity $d$, trading premium and disclosure costs against a residual-risk penalty $\lambda R\,e^{-kd}$ whose bite the regulator scales through $\lambda$ and firms attenuate through disclosure. The interior optimum is a closed-form object: the unhedged fractions solve $2\Lambda(d)\,\Sigma\,(1-h)=p$, so the equilibrium hedge vector is $\mathbf{1}-\tfrac{1}{2\Lambda(d)}\Sigma^{-1}p$ with $\Lambda(d)=\phi+\lambda e^{-kd}$, and the disclosure first-order condition balances marginal disclosure cost $2ad$ against marginal penalty relief $k\lambda e^{-kd}R$. The model delivers signed, testable comparative statics -- $\partial h_f/\partial\lambda>0$ (robust), $\partial h_f/\partial a>0$ (a disclosure-cost-to-hedging substitution channel), and a correlation effect $\partial h_f/\partial\rho$ whose sign it pins down in closed form -- rather than asserting them; the sign of the disclosure response $\partial d^\star/\partial\lambda$ is itself governed by an explicit boundary, $\phi\gtrless\lambda e^{-kd^\star}$, because hedging and disclosure are substitutes in the penalty channel. A companion engine certifies the closed-form solver against an independent optimizer (agreement $\sim10^{-8}$), verifies every comparative-statics sign, and shows on model-generated staggered panels that the group-time estimator recovers the true treatment effect where two-way fixed effects is visibly biased -- and that the substitution prediction, real but second-order, is exactly the one a capability confound would overturn first, which is why the identification strategy is built to isolate it. On the empirical side I specify a staggered difference-in-differences design keyed to Korea's phased KSSB/ISSB-aligned disclosure mandate, with a heterogeneity-robust event study, an explicit parallel-trends test, and synthetic-control, triple-difference, and instrumental-variable alternatives, all built on a reproducible OpenDART/XBRL/KRX/FnGuide pipeline with fully specified derivative-notional, hedge-accounting-adoption, and risk-efficiency proxies -- the structured front of which is executed live here against the OpenDART API for a real fourteen-firm, four-year pilot that retrieves clean financials and parses the derivative notes, finding the hedge-accounting designation reliably readable (validated by hand for S-Oil) but the numeric derivative tables filed in incompatible types and units across firms, so that the adoption outcome is extractable at scale while the continuous hedging-intensity outcomes require firm-level table adjudication -- a construct-validity constraint the paper locates with live data rather than assuming away. The theory generates signed predictions; the identification strategy states, to the coefficient, what would confirm or falsify them, addressing treatment endogeneity, notional-versus-fair-value measurement, and the mapping from disclosure text to hedging substance directly in its design rather than as after-the-fact qualification.
\end{abstract}
...
Korea is an unusually clean laboratory for this question. The Korea Sustainability Standards Board (KSSB) issued ISSB-aligned disclosure standards (mirroring IFRS~S1 and~S2) with a phased adoption schedule that first binds the largest KOSPI constituents and later widens to smaller issuers. Phased adoption by size and listing tier generates precisely the staggered treatment structure that modern difference-in-differences (DiD) estimation is built to exploit, with a large stratum of not-yet-covered firms serving as controls. Korea also has a centralized, machine-readable filing system -- the Financial Supervisory Service's OpenDART, with XBRL-tagged financial statements and structured derivative and hedge-accounting footnotes -- that makes the required firm-level panel constructible and reproducible rather than hand-collected.
...
This paper is a research \emph{design}: a closed-form theory paired with an identification strategy specified to the coefficient, rather than an executed empirical study on real firms. Its testable content is the set of signed predictions the model produces and the specification that would confirm or falsify them; a companion engine (Section~\ref{sec:numerics}) certifies the closed-form solver against an independent optimizer, verifies every comparative-statics sign, and shows on model-generated staggered panels that the design's estimators recover the predicted effects, establishing the internal logic that the empirical design then carries to real filings. Section~\ref{sec:robust} builds the design directly around its principal identification threats -- endogenous timing of the mandate, notional-versus-fair-value mismeasurement of hedging, staggered-adoption bias, and functional-form dependence -- with the identification and measurement choices of Sections~\ref{sec:design}--\ref{sec:data} constructed specifically to address each.
...
C_D=a\,d^2,\qquad a>0,
\end{equation}
capturing the increasing marginal cost of producing and assuring incremental ESG/climate disclosure. Residual risk carries a baseline economic cost -- the distress- and financing-friction cost of Froot--Scharfstein--Stein --
\begin{equation}
...
\subsection{Second-order conditions, existence, and uniqueness}
\label{sec:soc}
...
The outcome variables the design most cares about -- derivative notional/fair value by risk type and the cash-flow-hedge designation -- are \emph{not} in the structured account endpoint; they live in the filing notes. That layer was not merely probed but \emph{parsed}: for all fourteen firms the FY2023 annual report (\texttt{saeop-bogoseo}) was located through \texttt{list.json}, retrieved through \texttt{document.xml}, and its derivative note parsed -- reading the hedge-accounting designation from the note's operative sentence, and the derivative figures from the note's tables. The exercise turns the paper's abstract caveat into three concrete, measured findings (Table~\ref{tab:parseresults}, Figure~\ref{fig:realdata}).
...
\paragraph{Functional-form dependence.} The penalty $\lambda R e^{-kd}$ is a reduced form, and the design does not want its conclusions to hinge on it. The comparative-statics signs of Section~\ref{sec:compstat} depend only on $\partial P/\partial d<0$ and $\partial P/\partial R>0$, not on the exponential kernel: replacing $e^{-kd}$ with any smooth decreasing attenuation (e.g.\ the power law $(1+kd)^{-1}$) preserves every sign, as noted at \eqref{eq:penalty}. The structural extension (Section~\ref{sec:extensions}) additionally estimates $(\lambda,k,a,\phi)$ rather than fixing them, so the functional form becomes a testable object rather than an assumption.
...
This paper's predictions are model-derived and machine-verified rather than estimated on real firms; its empirical content is the specification and falsification structure that a coefficient table would test, not the coefficient table itself. The theory collapses the multi-factor commodity/currency exposure of the companion papers into a single financial factor $\sigma_f$ to keep the equilibrium closed-form -- the two-factor structure of Park (2026a) extends the model at the cost of a $3\times3$ system -- and the penalty $\lambda R e^{-kd}$ is a reduced form for cost-of-capital and reputational frictions the model does not micro-found. Two further gaps between the two-risk theory and the estimable panel are handled explicitly rather than papered over. First, the climate hedge ratio $h_c$ is empirically sparse: outside the K-ETS-covered emitters most firms disclose no quantifiable climate-hedging instrument, so the full-panel tests estimate the financial-risk special case and the two-risk model is identified only on the emitter subsample (Section~\ref{sec:mapping}) -- the reduction is nested and stated, not silent. Second, the disclosure-response boundary \eqref{eq:ddboundary} turns on the latent distress-cost parameter $\phi$, which would render the sign of $\partial d^\star/\partial\lambda$ unfalsifiable if $\phi$ were left unobserved; H5 (Section~\ref{sec:hypotheses}) restores falsifiability by proxying $\phi$ with observable distress-cost measures and testing the implied cross-sectional \emph{ordering} of the disclosure response, but the rescue is only as good as those proxies, and a reader should treat the disclosure-response prediction as the design's most fragile. The identifying variation is a single country's phased mandate, so external validity beyond ISSB-aligned regimes is a question for future extensions rather than this design; derivative-notional measurement is coarse and hedge-accounting election is observed only when disclosed; and the first treatment phase contains few, large, and idiosyncratic firms, which is why synthetic control and wild-cluster-bootstrap inference are built into the design rather than added as robustness checks. The triple-difference and placebo-timing tests of Section~\ref{sec:design} are constructed specifically to separate the mandate's effect on hedging from a coincident shift in hedging demand driven by the same climate developments that motivated the mandate.

Executing this design on real filings is principally a data-construction task rather than a theoretical or econometric one, and the executed pilot of Section~\ref{sec:pilot} now locates that task precisely -- and, in one respect, revises the design's own optimism. The closed-form solver, the comparative-statics signs, and the staggered-DiD estimator are all machine-verified against model-generated panels (Section~\ref{sec:numerics}); the identification strategy is specified to the coefficient; and the structured front of the pipeline was run live against OpenDART, returning clean financial fields for a real fourteen-firm, four-year sample. Parsing the derivative notes for those firms, however, surfaces a structural obstacle that is not a matter of parser effort: the hedge-accounting \emph{designation} is reliably readable from the note's operative sentence (resolved for ten of fourteen pilot firms, and validated by hand for S-Oil), but the \emph{numeric} derivative tables are filed in incompatible types and units across issuers -- fair-value, notional, and profit-and-loss-impact tables, in KRW and USD, sometimes several within one firm -- so that only one of thirteen located notes is directly comparable (Section~\ref{sec:pilot}). The implication for the empirical program is concrete: the hedge-accounting-adoption outcome (H2) is extractable at scale, but the continuous hedging-intensity outcomes (H1, H4) require firm-level table adjudication or a supervised extraction model rather than a uniform parse, and a full execution should budget for that explicitly. That the binding constraint is disclosure heterogeneity and construct validity -- not the economics or the econometrics -- is the honest lesson of the pilot, and it is where a careful execution of this design will succeed or fail.

% =============================================================================
\section{Conclusion}
\label{sec:conclusion}

Whether mandatory ESG disclosure changes corporate hedging is, at bottom, a question about the price a firm puts on the risk it chooses not to hedge. This paper makes that intuition precise: in a joint model of hedging and disclosure, disclosure enters only through the effective residual-risk price $\Lambda(d)=\phi+\lambda e^{-kd}$, the interior optimum in the two hedge ratios is the closed-form minimum-variance vector $\mathbf 1-\tfrac{1}{2\Lambda(d^\star)}\Sigma^{-1}\mathbf p$, and the disclosure choice balances quadratic disclosure cost against exponential penalty relief at a unique, globally optimal fixed point. From that structure the paper \emph{derives} its testable content -- a stricter regime raises both hedging and disclosure ($\partial h_f/\partial\lambda>0$, $\partial d/\partial\lambda>0$); costlier disclosure paradoxically raises hedging through substitution ($\partial h_f/\partial a>0$); and the correlation between financial and climate exposure moves the optimal hedge by a sign the model pins down in closed form. The empirical half converts these into a staggered, heterogeneity-robust difference-in-differences on Korea's phased KSSB/ISSB mandate, with an explicit parallel-trends test, synthetic-control, triple-difference, and instrumental-variable alternatives, and a fully specified OpenDART/XBRL/KRX/FnGuide pipeline -- together with a discriminating falsification test, H(sub), whose sign only the structural penalty channel produces. The paper is deliberately a design: it states what the theory predicts, what data would confirm it, and what pattern would refute the mechanism rather than merely fail to support it. Placed alongside its three companions -- static allocation, dynamic hedging, and hedge accounting of the same underlying position -- it supplies the missing first link: what makes the firm want to hedge at all.
