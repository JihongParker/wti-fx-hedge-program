...
The forward test therefore sharpens from a single post-period sign into a dated \emph{path} prediction: the hedging response should be muted in the first reporting years and concentrate around 2030---2031, when the penalty and verification parameters bind, a pattern the event-study apparatus of Section~\ref{sec:eventstudy} is already shaped to trace and one that a flat path through 2031 would falsify. Figure~\ref{fig:ladder} draws both halves: the executed nulls on the lower rungs against the predicted-positive climate rung, and the regime's calendar with the predicted response window.
...
These considerations condition how the paper's principal structural result should be read. The comparative statics of Section~\ref{sec:compstat} follow from the implicit function theorem applied to the model's first-order conditions, and their signs are therefore consequences of the assumed functional forms and not empirical findings. That the Korean nulls of Section~\ref{sec:executed} are consistent with the model's prediction for a floor raised on a margin orthogonal to $\Lambda$ is supportive, but a null result is weak evidence for any particular structure: many models, including ones with no penalty channel at all, predict no hedging response to a governance-report mandate. Consistency between a structural prediction and an estimated coefficient does not establish that the mechanism generating the data is the one the model posits, particularly where the outcome is a null and the data carry the measurement and endogeneity problems described above. The forward test on the climate mandate, which the model predicts should move hedging and not leave it unchanged, is the more informative one for this reason, and the interpretation offered here should be treated as provisional until that test is run.
