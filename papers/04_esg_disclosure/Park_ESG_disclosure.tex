...
\centering
\small
\begin{tabular}{p{4.0cm} p{6.6cm} p{2.6cm}}
\toprule
...
Before any real filing is parsed, two things can and should be checked: that the equilibrium of Section~\ref{sec:model} is what the closed form claims, and that the estimators of Section~\ref{sec:design} recover the predicted signs when the data are generated by the model itself. A dedicated engine does both. Its scope is internal (it validates the design's logic on model-generated data and is \emph{not} evidence about real firms), but the discipline is the right one: certify the machinery on cases with a known answer before trusting it on cases without one.
