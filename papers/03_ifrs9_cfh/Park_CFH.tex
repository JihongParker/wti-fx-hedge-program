...
The underlying quanto KO product -- its jump-diffusion dynamics, its Longstaff--Schwartz regression pricing surface, and its double-barrier touch mechanics -- is specified fully in Section~\ref{sec:product} of this paper; it is the same product studied from a dynamic-hedging perspective in a companion analysis, but this paper's entire contribution is on top of that pricing layer, in the accounting-designation question, and Section~\ref{sec:product} is written to be self-contained.
...
\end{align}
with $N(t)$ a Poisson process of intensity $\lambda$ and log-jump-sizes $Y_j \overset{\text{iid}}{\sim}\mathcal N(\theta_J,\delta_J^2)$. The production calibration (shared with the companion dynamic-hedging analysis) fits $\lambda=6.7846$ jumps per year, $\theta_J=-0.0299$, $\delta_J=0.0844$, a diffusive WTI volatility $\sigma_1=0.32419$, an FX volatility $\sigma_2=0.09258$, and a WTI--FX correlation $\rho=0.0876$.
...
Total inception premium outflow is KRW 34,264,941,589, matching Structure~A's own mean inception fair value to within rounding -- a direct internal consistency check that the engine's premium booking and its own fair-value engine agree at $t=0$.
