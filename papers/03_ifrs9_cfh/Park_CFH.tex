...
\begin{abstract}
A Korean crude-oil importer's commodity-and-currency exposure can be hedged under IFRS~9 either as one combined instrument (a quanto knock-out call) or as two independent risk-component designations. This paper builds a full path-wise cash-flow-hedge accounting engine to quantify how the two architectures differ on hedge ineffectiveness, OCI dynamics, and the economic risk surviving a knock-out event. The combined designation's mean ineffectiveness comes out roughly three-and-a-half times larger than the split designation's, almost entirely from the split structure's commodity leg and not its currency leg, while full-horizon economic risk differs by only $1.3\%$ of level. Extending the comparison to barrier-free variants of both designations isolates the driver: the split leg's ineffectiveness falls to a level indistinguishable from zero regardless of the barrier, while the combined structure's stays several times larger in the barriered configuration ($24.78$ against $7.70$bn, a factor of $3.2$) and does not approach zero in the barrier-free one, so what drives its elevated ineffectiveness is multiplying two floating exposures into one payoff, not the knock-out feature itself. Every headline figure is traced directly to the accounting engine's own live computation, and the SFP balance is confirmed at every step, though by construction of the pro-forma rather than as an independent test; the flow-level decomposition that would be an independent test instead fails, by an amount equal to the reclassification the knock-out sweep produces, and is reported as a result. The paper argues that the ineffectiveness gap is better understood as a consequence of IFRS~9's hypothetical-derivative convention than as a difference in economic risk.
\end{abstract}

\vspace{0.5em}
\noindent\textbf{Keywords:} IFRS 9; cash flow hedge; hedge effectiveness; hypothetical derivative; designation architecture; knock-out barrier; FVTPL reclassification

\vspace{0.3em}
\noindent\textbf{JEL classification:} M41; G32; G13; M48
...
\end{align}
with $N(t)$ a Poisson process of intensity $\lambda$ and log-jump-sizes $Y_j \overset{\text{iid}}{\sim}\mathcal N(\theta_J,\delta_J^2)$. The production calibration fits $\lambda=6.7846$ jumps per year, $\theta_J=-0.0299$, $\delta_J=0.08443$, a diffusive WTI volatility $\sigma_1=0.3242$, an FX volatility $\sigma_2=0.0926$, and a WTI---FX correlation $\rho=0.0876$, estimated on the 28 June 2021 to 22 June 2026 daily WTI and USD/KRW history (1{,}300 paired log returns) by a threshold-classification jump/diffusion decomposition (jump days flagged by a $k$-sigma rule, regimes moment-matched, diffusive volatility re-estimated from the remainder); the pricing rates are $r_{US}=0.040$ and $r_{KRW}=0.035$ throughout.
...
\end{equation}
where $\mathcal E_{\mathrm{jump}}\ge0$ collects paths a jump crosses the barrier on and then reverts into $[L,U]$ by $t_{i+1}$, indistinguishable from a jump-free interior path given only the two recorded endpoints. Since such a path needs both a jump landing within an interval of length $\Delta t$ ($O(\lambda\Delta t)$) and that jump clearing a barrier several diffusive standard deviations away, $\mathcal E_{\mathrm{jump}}=O(\Delta t)\to0$ as $\Delta t\to0$, a small residual gap-risk understatement of the true continuous-time KO frequency. Its size is measured directly: a jump-adapted exact re-simulation at $200{,}000$ paths (Poisson jump times drawn exactly within each step, per-sub-interval bridge tests, barrier checked at every jump instant, with the production-style whole-step test evaluated on the \emph{same} paths) puts $\mathcal E_{\mathrm{jump}}$ at $+0.14$ to $+0.18$ percentage points of KO frequency at this daily grid (risk-neutral and physical banks respectively), confirming the $O(\Delta t)$ analysis with a number (Section~\ref{sec:limitations}).
...
\end{equation}
Fair value along a path is $\max(\tilde C(S_1,S_2)\cdot\mathrm{barrels},\,\mathrm{intrinsic})$ from Structure~A's own regression surface; the floor at intrinsic value is a numerical safeguard described in Section~\ref{sec:safeguards}, where its bias near the upper barrier is also recorded. On knock-out, $V_A\to 0$; the CFH reserve freezes and any unamortized time value is swept to ineffectiveness (Section~\ref{sec:ko}).
...
\Delta\mathrm{CFHR}_A &= \mathrm{SignedLowerOf}\big(\Delta V_A-V_{A,0},\ h_t\cdot(H^\star_t-H^\star_0)\big). \label{eq:cfhr}
\end{align}

\paragraph{What $h_t$ is, dimensionally.} The $\beta$ coefficients carry units of KRW per unit of the structure, and the denominator $S_1(0)S_2$ is (USD/bbl)$\times$(KRW/USD) $=$ KRW/bbl, so $\mathrm{rawH}$ has dimensions of barrels. Multiplied by $H^\star$, which is in KRW, the product in \eqref{eq:cfhr} would carry bbl$\cdot$KRW and could not be compared with $\Delta V_A$ in KRW. The clip to $[0,1]$ is what reconciles this: after clipping, $h_t$ is used as a dimensionless coverage fraction, which is also what IFRS~9 means by a hedge ratio, since the standard defines it as a quantity relationship between hedging instrument and hedged item. The engine therefore computes a quantity that originates as a delta and applies it as a ratio. This is a deliberate choice and not an oversight, but it should be stated plainly, because $h_t$ enters the lower-of test directly and the headline KRW 23.36bn is a function of it. Two consequences follow. Values of $\mathrm{rawH}$ outside $[0,1]$ are truncated rather than rescaled, so the mapping from delta to coverage is not one-to-one at the boundaries; and the lower clip removes the negative sensitivities an up-and-out call genuinely carries near the barrier. A specification that normalized $\mathrm{rawH}$ by the structure's own barrel notional instead of truncating it would be dimensionally cleaner, and the difference between the two is not measured here.
Because $\beta_0(stp),\ldots,\beta_5(stp)$ are the same coefficients used to compute $V_A$ from Section~\ref{sec:lsmc}'s surface, $h_t$ is a genuine $\partial V/\partial S_1$-type sensitivity fixed at the start of the step, independent of that step's realized $V_A$; the system would be circular only if $\Delta\mathrm{CFHR}_A$ were \emph{defined} as $h_t\Delta H^\star$ and not \emph{constrained} against it by the lower-of operator above, and it is not.
...
\begin{itemize}
\item \textbf{Intrinsic floor.} The fitted WTI regression value is floored at intrinsic value, $\max(\tilde C\cdot\mathrm{barrels},\,\mathrm{intrinsic})$, at every step, not only at expiry, preventing local polynomial undershoot from driving the fitted surface below intrinsic. The safeguard is a numerical one and is described as such here: for a \emph{barrier-free} call the floor is a genuine no-arbitrage bound, but an up-and-out call may legitimately trade below intrinsic, because a spot level deep in the money is also a spot level close to extinction. At $S_1=119$ against $U=120$ and $K=78.94$ the intrinsic value is large while the contract is nearly certain to die, and the floor overstates the fair value there. How often this matters has been measured rather than left open. On a $200{,}000$-path bank the floor binds on $1{,}811{,}721$ of $17{,}213{,}700$ hedged nodes, $10.5\%$ of the total, lifting the fitted value by a mean $4.7\%$ of intrinsic where it binds. The distribution of those nodes is the point: within ten percent of the upper barrier the floor binds on $950{,}566$ of $950{,}966$ nodes, or $99.96\%$. The safeguard is therefore not occasional and not incidental. It is active on essentially every node in the region where it is theoretically invalid, and it lifts $V_A$ and $V_{B1}$ there, in the direction of understating measured ineffectiveness on paths approaching knock-out. Section~\ref{sec:awareness2} states what follows for the reported figures.
\item \textbf{Unambiguous regression-coefficient extraction.} The fitted regression coefficients are read from a fixed, validated array orientation at every step and not inferred, eliminating a class of silent coefficient misreads that an ambiguous read would otherwise permit.
\end{itemize}
...
\end{equation}
Per-path cumulative sums telescope:
\begin{equation}
\label{eq:telescope}
\sum_{i=1}^{n}\Delta E_A^{(i)} = \big(V_A(T)-V_A(0)\big)-\big(\mathrm{Phys}(T)-\mathrm{Phys}(0)\big),
\end{equation}
and identically for $B$, a direct consequence of summing a telescoping first difference, which follows by construction. Full-horizon dispersion is measured on the \emph{terminal path cumulant} $E_A^{\mathrm{path}}=\sum_{i}\Delta E_A^{(i)}$, not on the per-step cross-sectional standard deviation at any single step:
\begin{equation}
\label{eq:sigmaecon}
...
One convention is fixed here for the whole paper. Structure~A's premium of KRW 34{,}264{,}941{,}589 is the American-exercise LSMC valuation of the combined quanto knock-out, KRW 17{,}132.47 per barrel on 2{,}000{,}000 barrels, produced by the production LSMC pricing engine. The engine walks the accounting ledger on the same American surface, so where Section~\ref{sec:structA} describes the instrument as European-style it is describing the payoff form and not the exercise convention used in valuation.
\label{sec:inception}
...
\midrule
0.10 & 38.71 & 40.55 & 4.75 & 1.14 \\
0.50 & 33.22 & 33.08 & 0.42 & 4.74 \\
0.90 & 18.26 & 19.35 & 5.97 & 10.01 \\
\bottomrule
\end{tabular}
\end{table}

The independent reprice uses a reduced sample of $n_{\mathrm{sens}}=10{,}000$ LSMC paths and not the production surface; validation points are evaluated at $\tau/T\in\{0.1,0.5,0.9\}$ and not including $\tau/T=0$, since the inception point is a distinct case, separately and directly verified against the booked premium (Section~\ref{sec:inception}) and not by evaluating the fitted regression surface at a stressed test spot. Agreement is within 5\% for Structure~A throughout and for Structure~B1 up to mid-life, widening to roughly 10\% for B1 near expiry, consistent with ordinary finite-sample reprice noise on a shrinking, more path-dependent regression sample and not a systematic bias in either direction.
...
Structure~A's mean cumulative $|$ineffectiveness$|$ is KRW 23.36 billion versus Structure~B's KRW 6.40 billion. The statistic is $\mathbb E[\,|\sum_i \Delta\mathrm{Ineff}^{(i)}|\,]$, the cross-path mean of the absolute terminal cumulant, not the absolute value of the mean; Structure~B's figure is the signed combination of its two legs, $-6.57$ and $+0.18$, summed before rounding to $6.40$. Figure~\ref{fig:ineff} plots the cross-path \emph{mean} of the signed cumulant, which is why its Structure~A endpoint reads below the tabulated figure: signed cancellation across paths reduces the mean relative to the mean absolute, roughly three-and-a-half times larger. With the tautology-avoiding HD of Section~\ref{sec:hd} in place, this does not arise from a degenerate zero-ineffectiveness setup: it reflects that $H^\star$ continues tracking the physical exposure through knock-out while $V_A$ is extinguished, so every KO path contributes a large, non-cancelling ineffectiveness term to Structure~A that Structure~B, whose two legs are tested separately and whose FX leg survives KO, does not generate to the same degree (Figure~\ref{fig:ineff}).
...
Full-horizon $\sigma_{\mathrm{econ}}^A=$ KRW 104.52 billion and $\sigma_{\mathrm{econ}}^B=$ KRW 103.18 billion, a difference of roughly 1.3\%, well within ordinary Monte Carlo re-seeding noise observed across repeated engine runs (independent re-executions of the identical engine produced $\sigma_{\mathrm{econ}}^B$ values ranging from KRW 102.4 to KRW 103.2 billion). The two architectures are, empirically, economically indistinguishable at the full-horizon aggregate level despite the several-fold difference in accounting ineffectiveness reported in Section~\ref{sec:h1}: direct evidence that accounting effectiveness and economic residual risk measure genuinely different quantities. Because the two architectures run on a shared path bank, the comparison can be made per path rather than across runs, and it has been. On $200{,}000$ paired paths the validated re-implementation returns $\sigma_{\mathrm{econ}}^A=$ KRW 107.01bn against $\sigma_{\mathrm{econ}}^B=$ KRW 104.40bn, and the paired difference is unambiguous: $\mathbb E[|E_A|-|E_B|]=$ KRW 9.76bn with a standard error of KRW 0.09bn, a $t$-statistic of $103.9$, and a paired test of equal variance rejecting at $t=145.0$. The two architectures do \emph{not} carry statistically equal economic risk, and any statement to that effect would be an artifact of comparing cross-run re-seeding dispersion instead of the paired difference the shared bank makes available.

What survives, and what the paper's argument actually needs, is the economic magnitude rather than the statistical one. Structure~A's residual risk exceeds Structure~B's by $2.5\%$ of level on the re-implementation and by $1.3\%$ on the production ledger. Set against a recognized-ineffectiveness difference of several hundred percent between the same two architectures, a two-percent difference in residual risk is not what the accounting is responding to. The claim made throughout this paper is therefore the ordering-of-magnitudes one, a claim about economic size rather than about statistical indistinguishability.
...
\caption{B1 (WTI) and B2 (FX) mean fair value over time. B2 freezes at $t=T_{FX}=0.5$, independent of each path's WTI knock-out status.}
\label{fig:freeze}
\end{figure}

The freeze is unconditional: from step 131 (the first step at or beyond $T_{FX}$) onward, B2's cross-path mean fair value is constant at every subsequent step, KRW 8.638 billion, for every path regardless of whether that path's WTI leg has separately been knocked out. Freeze completion is gated on FX-leg maturity alone and not on WTI knock-out survival, so paths already knocked out before $T_{FX}$ freeze their B2 mark at $T_{FX}$ just as surviving paths do, and the cross-path mean shows the sharp plateau visible in Figure~\ref{fig:freeze}: every path contributes the identical frozen value from $T_{FX}$ onward, so the average is that value, with no smoothing from asynchronous freeze timing across paths.

This same unconditional gating has a direct consequence for the post-KO FVTPL statistic of Section~\ref{sec:h4}. That statistic pools two populations the freeze mechanism makes structurally distinct. For a path with $\tau_{KO}<T_{FX}$, B2 is still marking to fair value when the WTI barrier fires, is reclassified to FVTPL while still live, and continues generating genuine FVTPL volatility until it separately freezes at $T_{FX}$, the ``zombie window'' Section~\ref{sec:h3} describes. For a path with $\tau_{KO}\ge T_{FX}$, B2 has \emph{already} frozen, on its own contractual schedule, before the WTI barrier ever fires; its fair value is unchanged through and past the knock-out event, so $\mathrm{FVTPL}_B$ accrues no further profit or loss for the remainder of that path, by the same arithmetic that produces the plateau in Figure~\ref{fig:freeze}: reclassification to FVTPL happens on paper, but there is no leg left alive to generate any P\&L from it. The $200{,}000$-path exact re-derivation of Section~\ref{sec:h4} finds close to an even division of the knocked-out population ($52.6\%$ knock out before $T_{FX}$, $47.4\%$ after); and, as the freeze mechanism predicts, zero post-KO FVTPL mean and zero standard deviation on every path in the $\tau_{KO}\ge T_{FX}$ subgroup, with the entire distribution's dispersion concentrated in the $\tau_{KO}<T_{FX}$ subgroup (mean KRW 0.53 billion, standard deviation KRW 7.08 billion on that subgroup alone). The pooled headline is therefore not a description of what a ``typical'' knocked-out path experiences; it is a mixture of a subpopulation that experiences the full zombie-FVTPL mechanism and a same-sized subpopulation that experiences none of it, already inert by the time its designation status changes: both the population split and the zero result follow from the freeze mechanism itself.

% =============================================================================
\section{Sensitivity}
\label{sec:sensitivity}

\paragraph{Reprice sample size.} The independent LSMC reprice sample used for Table~\ref{tab:fvval} is $n_{\mathrm{sens}}=10{,}000$ paths. The A~\% diff column is computed from the two level columns beside it, so the table is internally consistent; an earlier draft carried a \% column from a separate reprice run, which is why Figure~\ref{fig:fvval} and this column now agree only up to reprice noise. The $\tau/T$ index is elapsed time fraction, so $\tau/T=0.9$ is near expiry. Near-expiry validation error is single-digit percent for Structure~A throughout, and for Structure~B1 up to mid-life, widening only modestly near expiry, consistent with ordinary Monte Carlo reprice noise near a payoff kink at this sample size and not a systematic bias in either direction.

\paragraph{Static vs.\ dynamic hedge ratio.} Fixing $h_t\equiv h_0$ at its inception value would remove the regression surface's sensitivity to moneyness ($2\beta_3v_1$) and cross-asset correlation ($\beta_5v_2$) from Structure~A's effectiveness test; the production engine recomputes $h_t$ every step from Section~\ref{sec:ht}, and a full static-ratio counterfactual run is left for future work. This is the single most useful robustness check not performed here: because $h_t$ enters \eqref{eq:hstar}'s comparison directly, the headline KRW 23.36bn is a function of the dynamic ratio, and a static-$h_0$ run would separate the mismatch attributable to the payoff's multiplicative form from the mismatch attributable to re-estimating the ratio each step.

\subsection{A validated fourth cell: the combination drives the gap}
\label{sec:vanillabench}

Structure~A is always a single instrument \emph{and} always barriered; Structure~B is always split \emph{and} its WTI leg (B1) is also always barriered. The main comparison therefore never isolates whether Structure~A's elevated ineffectiveness (Section~\ref{sec:h1}) comes from combining the exposure into one instrument, from the knock-out feature that instrument carries, or from both together. Separating a $2\times2$ design (combined-vs-split, barriered-vs-vanilla) requires two further structures beyond A and B: a combined quanto call with the barrier removed, and a split WTI-only call (FX fixed, B1's payoff) with the barrier removed.

Decomposing Structure~B's own ledger first, and not only citing its total, is the right place to start: terminal cumulative ineffectiveness splits KRW $-6.57$ billion for B1 against KRW $+0.18$ billion for B2 (frozen at $T_{FX}$, Section~\ref{sec:freezeresult}): essentially all of Structure~B's reported KRW 6.40 billion comes from B1, not B2. B1 carries the identical knock-out barrier as Structure~A, yet its ineffectiveness is barely a quarter of Structure~A's (6.57bn against 23.36bn), a $3.56\times$ gap between two barriered instruments differing only in whether FX is fixed or floating.

Resolving that gap requires an engine faithful enough to reproduce it before it can be trusted to answer the harder question: any re-implementation that shows no combined-versus-split effect at all, when a $3.56\times$ gap demonstrably exists in production, has omitted something material and should not be relied upon for the cells production cannot itself compute. Extracting the production engine's mechanics line-by-line identifies the three features such fidelity requires: the hypothetical derivative $H^\star$ is a \emph{closed form}, $\mathrm{barrels}\cdot S_1\cdot S_2\cdot e^{\rho\sigma_1\sigma_2\tau}$, never a regression fit; Structure~A's lower-of test compares \emph{intrinsic}-value changes (not fair-value changes) against $h_t\cdot(H^\star_t-H^\star_0)$, the hypothetical scaled by the dynamic hedge ratio of Section~\ref{sec:ht}; and at the knock-out step the remaining unamortized cost-of-hedging is swept directly into that step's ineffectiveness. A re-implementation faithful to all three mechanics, at daily (217-step) monitoring, in a two-stage design matching production (fit $\mathrm{Beta\_Mat}$ on one path bank, walk a separate fresh accounting bank forward against the frozen surface), reproduces every available production statistic to within 1---17\% of the corresponding production figure: Structure~A ineffectiveness 24.78bn (production 23.36bn), Structure~B1 ineffectiveness $-7.70$bn (production $-6.57$bn), economic residual std 107.3bn/104.4bn (production 104.52bn/103.18bn), and critically, an A-to-B1 ratio of $3.22\times$ against production's $3.56\times$. Passing this validation check is what licenses trusting its answer to the fourth cell.
...
The result reverses the natural first guess entirely. \textbf{Split + vanilla is indistinguishable from zero.} B1's lower-of test compares option intrinsic value, $\mathrm{barrels}\cdot\max(S_1-K,0)\cdot S_2(0)$, against a fixed-notional WTI forward, $\mathrm{barrels}\cdot(S_1-S_1(0))\cdot S_2(0)$. These are the identical function of $S_1$ whenever the option is in the money, and the sign-consistency rule of Section~\ref{sec:lowerof} resolves the out-of-the-money case to zero as well: a single-asset option's intrinsic value cannot help but track a forward on the same asset, with or without a barrier. \textbf{Combined stays large in both barrier states.} Structure~A's hedged item is the \emph{product} of two floating quantities, $\max(S_1-K,0)\cdot S_2$; no forward-style hypothetical, scaled or not, is that same function of $(S_1,S_2)$, so the mismatch never mechanically vanishes the way B1's does, in either barrier state.

The mismatch can in fact be named exactly, which is worth doing because it converts an empirical regularity into an analytic one. On paths that are alive and in the money, Structure~A's intrinsic value decomposes as
\begin{equation}
\mathrm{barrels}\cdot(S_1-K)\,S_2 \;=\; \underbrace{\mathrm{barrels}\cdot S_1S_2}_{\;=\;H^\star\ \text{by }\eqref{eq:hstarsimple}} \;-\; \mathrm{barrels}\cdot K\,S_2 ,
\label{eq:kleg}
\end{equation}
so the hedged item and the hypothetical derivative differ by precisely the term $-\mathrm{barrels}\cdot K\cdot S_2(t)$: the currency exposure of the \emph{strike} leg. Structure~B1 fixes the conversion at $S_2(0)$, which makes that term a constant and therefore cancels it out of every first difference; Structure~A leaves it floating, so it survives into $\Delta V_A$ and has no counterpart in $\Delta H^\star$. This is the mechanism behind the $3.2\times$ gap, stated more sharply than ``because the payoff is a product''.

The magnitude checks out. The strike leg carries $\mathrm{barrels}\cdot K=\$157{,}880{,}000$, worth KRW 243.2bn at inception, and $\sigma_2\sqrt{T_{WTI}}=8.45\%$, giving a one-standard-deviation currency swing on that leg of about KRW 20.6bn against the measured KRW 23.36bn. The correspondence is not an exact attribution, since $h_t<1$ scales the comparison and the lower-of test truncates it, but the order and the sign are those of \eqref{eq:kleg}, and no other term in the decomposition is of that size. The combined-vanilla cell's \emph{absolute} level, however, is the one cell with no production anchor and proves sensitive to re-implementation details (fit safeguards, path law, sample size move it across a wide range while every other cell's ordering is stable), so this paper draws no conclusion about whether removing the barrier raises or lowers the combined structure's ineffectiveness: only that it never approaches the split structure's mechanical zero.

\textbf{Structure~A's ineffectiveness is therefore driven by the combination of two floating exposures in one multiplicative payoff, and not by the barrier.} For the split structure the relationship is reversed: B1's ineffectiveness is negligible in the absence of a barrier and is created almost entirely \emph{by} the barrier's knock-out-day cost-of-hedging sweep. Economic risk responds to the barrier in the opposite direction from accounting ineffectiveness: combined+vanilla's residual-risk standard deviation (42.3bn) is far below combined+KO's (107.3bn), since removing the barrier removes the total-loss-of-coverage-on-knock-out mechanism of Section~\ref{sec:h2}. Whether a structure is barriered and whether it is combined therefore pull accounting ineffectiveness and economic risk in opposite directions, and Section~\ref{sec:discussion} returns to what this means for reading the two structures' numbers together.

% =============================================================================
\section{Discussion}
\label{sec:discussion}

The accounting architecture of this paper rests on a single balance, $V(t)=\mathrm{OCI}(t)+\mathrm{RE}(t)$, which Appendix~\ref{sec:appendixB} confirms at every step and which holds by construction of the pro-forma rather than as an independent test of the engine. The check with content is the flow-level decomposition, which fails once knock-outs begin by an amount equal to the KRW 24.41 billion reclassification: Appendix~\ref{sec:appendixB} reports that gap as a finding. OCI is where the effective, well-behaved part of a hedge's fair-value movement is parked (a reserve, not an income statement line) alongside the deferred cost-of-hedging (Section~\ref{sec:coh}) amortizing quietly in the background. P\&L is where the lower-of test's \emph{ineffective} residual is required to land the moment it is measured, whether or not the firm has done anything wrong: it records how exactly a specific hypothetical derivative happens to track a specific instrument, and is better read as a measurement of that tracking convention than as a report of hedging skill. Retained earnings is where both amortized cost-of-hedging and every recognized ineffectiveness charge finally accumulate: the one line that actually marks the firm's capital. The two architectures compared here do not disagree about total economic risk; full-horizon $\sigma_{\mathrm{econ}}$ differs by $1.3\%$ of level between them, KRW 104.52bn against 103.18bn, a paired difference that is statistically sharp but economically small (Section~\ref{sec:h2}). They disagree, sharply, about \emph{when} and \emph{how visibly} that risk is forced out of OCI's holding pattern and into P\&L and, from there, into retained earnings.

Structure~A's OCI reserve behaves normally for as long as the option survives: intrinsic-value changes track the barrier-free quanto forward reasonably well, cost-of-hedging amortizes on schedule, and the running ineffectiveness charge is modest. The break comes in one step, at the knock-out event. $V_A$ collapses to zero; the CFHR reserve freezes at its last value; and (a mechanic easy to miss in the ledger but decisive in the total) whatever cost-of-hedging has not yet amortized is swept directly into that same step's ineffectiveness and not carried forward (Section~\ref{sec:ko}). Section~\ref{sec:vanillabench}'s validated re-derivation locates \emph{why} this charge is so much larger than Structure~B's: the driver is that Structure~A's hedged item is the product of two floating quantities, $\max(S_1-K,0)\cdot S_2$, a function that no forward-style hypothetical derivative can replicate under any fixed scaling. Removing the barrier does not repair this mismatch: the split leg's zero is unreachable for a product payoff in either barrier state. The barrier's actual role is to concentrate the unavoidable mismatch into one visible, dated event. What does not vanish at that event is the frozen OCI reserve accumulated up to it (KRW 24.41 billion, Section~\ref{sec:h3}): IFRS~9's own mechanics direct it toward the acquired inventory's cost basis at physical settlement and not toward an immediate further P\&L charge, so Structure~A's capital-account damage is concentrated, but bounded, at a moment the firm can see coming as the barrier approaches.

Structure~B's day-to-day ledger looks better than Structure~A's for a more mechanical reason than the intuition that two smaller hedges are safer than one. B1's own lower-of test compares option intrinsic value against a fixed-notional WTI forward (the same function of $S_1$ whenever the option is in the money), so the two track each other exactly by construction; the validated re-derivation puts B1's ineffectiveness in the absence of a barrier at a number indistinguishable from zero, with essentially all of its production-observed KRW 6.57 billion coming from the barrier's own knock-out-day cost-of-hedging sweep, the same mechanic as Structure~A's, just smaller because B1 has no floating-FX term to fight. This clean-looking book is the point at which a CFO reading only the ineffectiveness line could be misled: it says nothing about B2. B2 carries no barrier, is unaffected by the WTI knock-out on its own terms, and continues marking to fair value via Garman---Kohlhagen for as long as it is designated, but two independent events end that designation early and separately. First, at its own contractual maturity $T_{FX}=0.5$, well before $T_{WTI}=0.833$, B2's hedge relationship is discontinued by maturity, not by any market event (Section~\ref{sec:freezeresult}). Second, and more consequentially for paths that survive past $T_{FX}$, the moment the WTI barrier fires, B2 is reclassified out of hedge accounting entirely and into FVTPL (Section~\ref{sec:h4}): not because anything happened to the FX rate, but because its \emph{partner} leg's discontinuation flag fired. From that point its fair-value changes flow straight into P\&L with no OCI buffer at all, and this is where Structure~B's real capital-account risk actually lives: post-knock-out FVTPL volatility with a standard deviation of KRW 5.14 billion pooled across all knocked-out paths, rising to KRW 7.08 billion on the roughly half of paths for which the zombie window is genuinely open (Section~\ref{sec:h4}), silently eroding retained earnings for the remainder of the horizon, well after the CFO's attention has moved past the quiet ineffectiveness line that looked so favorable earlier.

A stronger reading of the same evidence is worth stating explicitly, because it changes what the ineffectiveness numbers mean. The gap between Structure~A's and Structure~B's measured ineffectiveness is not a difference in economic risk. Section~\ref{sec:h2} finds full-horizon $\sigma_{\mathrm{econ}}$ differing by only $1.3\%$ of level across the two architectures, against an accounting gap of several hundred percent, and Section~\ref{sec:vanillabench} traces the ineffectiveness gap not to any feature of the underlying exposure but to the algebraic form of the hedged item relative to the hypothetical derivative the standard permits: a product of two floating quantities cannot be tracked by a forward-style hypothetical, while a single floating quantity can. Both structures hedge the same physical cargo, at the same notional, against the same two market factors. What differs is the designation boundary the firm draws around them and, consequently, which hypothetical derivative IFRS~9 obliges it to construct.

The hypothetical derivative is a stipulated benchmark and not a traded instrument, defined by the standard's own conventions (perfect terms-match, no counterparty credit, a specified treatment of time value and cost of hedging), and the ``ineffectiveness'' the lower-of test recognizes is the distance between an actual instrument and that stipulated object. That distance is a property of the measurement convention at least as much as of the hedge. On this reading, Structure~A's larger ineffectiveness is better described as an accounting artifact than as a discovered inefficiency: the multiplicative payoff behaves no worse economically than the split legs, while being less representable within the specific comparison the standard mandates. The result is a divergence between economic substance, which the two structures share, and accounting recognition, which separates them by a factor of several.

The causality therefore runs the other way than a direct reading of the accounting output would suggest. A firm that splits a designation has selected an architecture whose hedged item is representable by an admissible hypothetical derivative; its underlying risk is unchanged. The measured ineffectiveness ordering is real, auditable, and consequential for reported earnings, and for those reasons it is a genuine constraint on structuring decisions; but it is a constraint imposed by the reporting framework and not by the market. The practical implication is that the ineffectiveness line should be read as a designation-architecture diagnostic and not as a risk metric, and that any firm choosing between these structures on the basis of reported ineffectiveness alone is optimizing against the measurement convention and not against its exposure, which is precisely the behavior Section~\ref{sec:h4}'s post-knock-out FVTPL result shows can be costly.

There is also a price on the choice, and it has gone unreported. At inception Structure~A costs KRW 34.26bn, the premium of the single combined quanto. Structure~B costs KRW 35.62bn for the WTI knock-out call plus KRW 6.44bn for the Garman--Kohlhagen FX call, KRW 42.05bn in total: the split architecture is KRW 7.79bn, or $22.7\%$, more expensive. Set beside Section~\ref{sec:h2}'s finding that the two carry economic risk differing by $1.3\%$ of level, the practical statement is sharper than the ineffectiveness comparison alone suggests. A firm choosing Structure~B is paying roughly KRW 8bn for a reduction in recognized ineffectiveness that Section~\ref{sec:discussion} argues is an artifact of the hypothetical-derivative convention rather than a reduction in risk, and is buying, along with it, the post-knock-out FVTPL exposure of Section~\ref{sec:h4} that Structure~A does not carry. Stated that way the split designation is an expensive way to buy a quieter ineffectiveness line.

The choice between these architectures is therefore not a choice about how much risk the importer carries (both carry statistically the same economic risk), but about the shape of its accounting recognition. Structure~A reports a large, one-time, dated charge to P\&L at the barrier, after which its books go quiet: a single visible scar. Structure~B reports a small, reassuring ineffectiveness charge for most of the horizon, then hands an unbounded, undated, back-loaded P\&L bleed to whichever period the WTI barrier happens to fire in, through a leg that was never the one being watched. A single combined designation trades a bigger number for certainty about when it lands; two independent designations trade a smaller number for a tail risk that surfaces once the firm's attention, and its designation architecture, has already moved on.

% =============================================================================
\subsection{Effect of the Intrinsic Floor and the Ratio Clip}
\label{sec:awareness2}

Two engine conventions, the intrinsic floor of Section~\ref{sec:safeguards} and the $[0,1]$ clip on $h_t$ of Section~\ref{sec:ht}, act in the same region and in the same direction, and their measured extent bounds how much of the headline they can carry.

The floor binds on $99.96\%$ of hedged nodes within ten percent of the upper barrier and lifts fitted value by a mean $4.7\%$ of intrinsic where it binds. Because Structure~A's lower-of test compares $\Delta V_A$ against $\Delta H^\star$, and the floor raises $V_A$ on precisely the paths where the option is closest to extinction, the floor compresses the measured mismatch on those paths. Its removal would raise, not lower, Structure~A's recognized ineffectiveness; the reported KRW 23.36bn is therefore a lower bound with respect to this convention, and the several-fold A-over-B ordering that the paper rests on is not at risk from it.

The clip on $h_t$ works the other way but is smaller. On the same regression surface and calibration, the raw gradient is never negative over $17.3$ million nodes, so the lower clip never binds; the upper clip binds on $1.4\%$ of nodes. Re-running the delta hedge with the clip removed moves the hedge-cost standard deviation by four hundredths of one percent. On this evidence the clip is not a material determinant of the ledger, and the floor is the convention that warrants the caveat.

What neither measurement can do is establish the level of $V_A$ independently, because the same fitted surface supplies both the value and, through $h_t$, the ratio. The measured tendency of the fitted surface gradient to overstate $\partial V/\partial S_1$ by roughly an order of magnitude therefore transmits directly into $h_t$ here, and is the reason Section~\ref{sec:limitations} keeps the static-ratio counterfactual as the outstanding robustness check rather than treating $h_t$ as settled.

\section{Limitations}
\label{sec:limitations}

Several boundaries define this paper's scope. Monte Carlo estimates ($\mathrm{VaR}_{99}$ at $n=20{,}000$; near-expiry reprice validation at $n_{\mathrm{sens}}=10{,}000$) carry ordinary sampling noise, so the Structure~A/B naked-exposure $\mathrm{VaR}_{99}$ gap ($\sim$2\%) and the B1 near-expiry reprice discrepancy ($\sim$10\%) are order-of-magnitude comparisons and not point estimates. The KRW 24.41 billion reclassification figure of Section~\ref{sec:h3} is a cross-path mean; the amount on any individual path varies with knock-out timing (Appendix~\ref{sec:appendixB}). The Brownian-bridge barrier correction (Section~\ref{sec:barrier}) is exact between recorded steps and understates true knock-out frequency by a measured $+0.14$ to $+0.18$ percentage points at the daily grid, confirmed by a jump-adapted exact re-simulation at $200{,}000$ paths; the same exact engine directionally re-confirms the combined-versus-split ineffectiveness ordering that drives Section~\ref{sec:h1}'s headline result, so this paper's qualitative conclusions do not rest on the production scheme's discretization. WTI and FX maturities ($T_{WTI}=0.833$, $T_{FX}=0.5$) are reported as designed, reflecting the structures' actual contractual terms and not a counterfactual unified maturity. The $2\times2$ benchmark of Section~\ref{sec:vanillabench} and the post-KO FVTPL statistic of Section~\ref{sec:h4} run on a re-implementation that reproduces the production engine's mechanics and validates to within 1---17\% of every production statistic the current engine can still generate; the post-KO FVTPL cells and the barrier-free vanilla cells have no production analog to validate against by construction, so this paper reports its own measurement as the primary figure for those cells and not as a check against a production number. The representative knock-out path of Figure~\ref{fig:zombie} illustrates the qualitative mechanism of Sections~\ref{sec:h3}---\ref{sec:h4} on a single reconstructed path and is not a source of any reported cross-path statistic. Two engine conventions are quantified in Section~\ref{sec:awareness2} rather than left open. The intrinsic floor is a valid no-arbitrage bound only for a barrier-free call, and it binds on $99.96\%$ of hedged nodes within ten percent of the upper barrier, lifting fitted value by a mean $4.7\%$ of intrinsic there; because it raises $V_A$ on the paths closest to extinction it compresses the measured mismatch, so the reported KRW 23.36bn is a lower bound with respect to it and the A-over-B ordering is not at risk. The $[0,1]$ clip on $h_t$ is immaterial by measurement: the raw gradient never turns negative, the lower clip never binds, and removing the clip in the delta-hedge engine moves the cost standard deviation by $0.04\%$. What remains genuinely open is the static-ratio counterfactual, since $h_t$ inherits the measured surface-gradient level error. American early exercise, basis spreads, and counterparty credit risk are outside this paper's scope. A designation-architecture boundary concerns Structure~B's discontinuation mechanics. The engine routes both legs' post-knock-out treatment off a single flag triggered by the WTI barrier (Section~\ref{sec:structB}), while Section~\ref{sec:arch} designates B1 and B2 as two independent hedge relationships. Under IFRS~9.6.5.6 and B6.5.22---26 a relationship is discontinued only when its own qualifying criteria cease to be met, and B2's hedged item, the currency component of the forecast purchase, remains highly probable after the WTI barrier extinguishes B1's instrument; a strict reading therefore continues B2's cash-flow hedge on its own terms rather than terminating it with its partner leg. The shared flag embeds a joint-program discontinuation policy that is conservative for B2's OCI, routing B2's post-KO fair-value changes to FVTPL rather than OCI; continuing B2 through $T_{FX}$ would reallocate the post-KO FVTPL flow of Section~\ref{sec:h4} between OCI and P\&L and could move B2's small contribution ($+0.18$bn of Structure~B's 6.40bn total), while the A-versus-B ordering of Section~\ref{sec:h1}, carried by B1's 6.57bn against A's 23.36bn, does not rest on this convention. The continued-B2 counterfactual ledger is not run in this paper. The paper's central findings are stable across these boundaries: the combined designation's mean ineffectiveness is several times larger than the split designation's, the gap traces to the combined structure's multiplicative payoff and not to the barrier, and full-horizon economic risk differs between the two architectures by $1.3\%$ of level, a paired difference that is precisely estimated and economically small, against an accounting difference of several hundred percent. A final boundary is interpretive and not numerical. Every ineffectiveness figure reported here is defined relative to IFRS~9's hypothetical-derivative construct, and is therefore conditional on that construct's conventions and not on the economics of the hedge; a different admissible hypothetical, or a jurisdiction permitting a different effectiveness-assessment method, would produce different numbers from identical cash flows. The comparison between architectures is meaningful within the framework, and the paper makes no claim that the ineffectiveness ordering corresponds to an ordering of economic risk. Section~\ref{sec:discussion} argues, on the contrary, that it does not.

% =============================================================================
\section{Conclusion}
\label{sec:conclusion}

This paper measures the accounting consequences of two IFRS~9 cash-flow-hedge designation architectures for the same aggregated commodity-and-currency exposure. The hypothetical derivative of Section~\ref{sec:hd} removes Structure~A's tautological zero-ineffectiveness outcome; the dynamic hedge ratio of Section~\ref{sec:ht} is non-circular by construction; and the full-horizon economic-residual measure of Section~\ref{sec:decon} separates accounting effectiveness from economic risk explicitly. On $n=20{,}000$ simulated paths, Structure~A shows roughly three-and-a-half times larger mean cumulative ineffectiveness than Structure~B, while the two structures are economically indistinguishable on full-horizon residual risk and only modestly different on post-KO naked-exposure tail risk. Decomposing Structure~B further attributes essentially all of that smaller ineffectiveness to its WTI leg and not its FX leg, and a production-fidelity re-implementation validated to within 1---17\% of every reproducible production statistic traces the remaining gap to its source: combining two floating exposures into one multiplicative quanto payoff, not the knock-out barrier, is what defeats any forward-style hypothetical derivative's ability to track Structure~A, since a single-asset structure's intrinsic value tracks a forward almost by definition while a product of two floating prices cannot. The barrier's role is to concentrate this unavoidable mismatch into one dated, visible event and not to cause it. The choice between a single combined designation and two independent risk-component designations is, on this evidence, a choice about \emph{when} and how visibly recognized ineffectiveness reaches the firm's capital accounts (a front-loaded, bounded shock at a known event for Structure~A, versus a quiet ledger followed by an undated, back-loaded FVTPL bleed through an unwatched leg for Structure~B), not a choice about how much economic protection the importer ultimately retains.
...
Section~\ref{sec:hd} states $H^\star(t)$ in closed form; this subsection derives it from the same $\mathbb Q$-measure dynamics used throughout Section~\ref{sec:product}, so that the formula rests on the stated SDEs and not on assertion.
...
\end{equation}
The domestic risk-free rate appears nowhere in the final expression: it enters once as $S_2$'s $\mathbb Q$-drift and again, with opposite sign, as the discount rate, and the two cancel identically regardless of the rate's level. The surviving term $\rho\sigma_1\sigma_2\tau$ records the engine's measure specification rather than a quanto adjustment. Since the dynamics of Section~\ref{sec:sde} give $S_2$ the drift $r_{KRW}-r_{US}$, the simulation measure is $\mathbb Q^{KRW}$; under it $S_1S_2$ is a KRW-denominated traded asset and must drift at $r_{KRW}$, which forces
\begin{equation}
\mathrm{drift}(S_1) \;=\; r_{KRW}-(r_{KRW}-r_{US})-\rho\sigma_1\sigma_2 \;=\; r_{US}-\rho\sigma_1\sigma_2 .
\label{eq:driftcorrect}
\end{equation}
The engine specifies $r_{US}$ without the $-\rho\sigma_1\sigma_2$ term, and \eqref{eq:er1r2} carries that specification forward as the factor $e^{\rho\sigma_1\sigma_2\tau}$. Correlated shocks fix the joint distribution's shape, not its location; a drift adjustment is an artifact of the change of measure and must be imposed explicitly.

With \eqref{eq:driftcorrect} in force the algebra simplifies rather than complicates: $\mathbb E[R_1R_2]=e^{r_{KRW}\tau}$, the discount cancels it entirely, and the hypothetical derivative collapses to
\begin{equation}
H^\star(t) \;=\; \mathrm{barrels}\cdot S_1(t)\,S_2(t),
\label{eq:hstarsimple}
\end{equation}
with no correlation term at all. Numerically the two specifications differ by $e^{\rho\sigma_1\sigma_2\tau}-1=0.22\%$ of $H^\star$, and the sensitivity of the reported results to the choice is quantified directly: re-running the validated re-implementation on $200{,}000$ paths with the correlation factor removed from $H^\star$, per \eqref{eq:driftcorrect}, moves Structure~A's mean absolute ineffectiveness by $+0.76\%$, and no ordering, sign, or headline comparison changes. Equation \eqref{eq:er1r2} validates the algebra of the joint expectation; the measure specification is fixed by \eqref{eq:driftcorrect}. This is also what makes $H^\star$ fit for purpose under IFRS~9.6.3.4: the hypothetical derivative must benchmark the aggregated exposure's multiplicative WTI-times-FX risk linearly and without hedge-specific optionality, and a discounted $\mathbb Q$-expectation of the raw physical settlement is, by construction, a $\mathbb Q$-martingale carrying no barrier or other option-specific feature: exactly the property Section~\ref{sec:hd} requires of it.

\section{SFP Ledger Balance Verification}
\label{sec:appendixB}

A natural conjecture is that the derivative asset (debit), cost-of-hedging amortization and any P\&L loss (expense) sum to the OCI reserve (credit) and any P\&L gain (revenue) at \emph{every} step: a full double-entry decomposition of the accounting flows themselves, not merely of the resulting balances. Tested directly against the production ledger, that specific decomposition does not hold once knock-out events begin occurring across paths: the gap between the two sides grows from zero in the pre-knock-out period to a terminal value that equals, to the reported digit, the KRW 24.41 billion reclassification amount of Section~\ref{sec:h3}. That gap is a quantifiable signature of the residual-time-value knock-out sweep of Section~\ref{sec:structB} moving value from an implicit running balance into an explicit one-time reclassification event, and is reported here as a finding in its own right and not as the flow-level identity originally conjectured.

The identity that \emph{does} hold, verified directly against the ledger at every one of the 217 steps for both structures, is simpler and is enforced by construction in the SFP pro-forma output: the derivative asset's carrying amount equals the OCI reserve plus retained earnings,
\begin{equation}
\label{eq:sfpbalance}
V_A(t) = \mathrm{OCI}_A(t) + \mathrm{RE}_A(t), \qquad V_{B1}(t)+V_{B2}(t) = \mathrm{OCI}_B(t) + \mathrm{RE}_B(t),
\end{equation}
with retained earnings computed directly as $\mathrm{RE}=V-\mathrm{OCI}$ and not accumulated independently from signed P\&L line items. Measured against the production ledger the two sides agree to within $10^{-6}$---$10^{-8}$ KRW absolute at every tested step for both structures, against carrying amounts on the order of $10^{10}$---$10^{11}$ KRW.

That agreement should be read for exactly what it is, and no more. Because $\mathrm{RE}$ is \emph{defined} as $V-\mathrm{OCI}$ in the SFP pro-forma, \eqref{eq:sfpbalance} is true by construction and the $10^{-6}$ residual measures floating-point arithmetic rather than the accounting engine. It is a useful guard against transcription and aggregation faults in the pro-forma, and it is not an independent check on the ledger's economics: no sign-convention error in the P\&L attribution could cause it to fail. The check with genuine content is the flow-level decomposition in the paragraph above, and that one does \emph{not} hold once knock-outs begin. This paper therefore reports the flow-level gap as its substantive ledger finding, and treats \eqref{eq:sfpbalance} as a consistency guard on the output format.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_cfh_sfp_balance.pdf}
\caption{SFP debit (derivative asset, black outline) vs.\ credit (OCI reserve plus retained earnings, stacked area) over time, Structure A and Structure B. The outline traces the top of the stacked area at every step by construction (Eq.~\ref{eq:sfpbalance}). Structure B's visible kink near $t\approx 0.5$ is the maturity-freeze plateau of Section~\ref{sec:freezeresult}.}
\label{fig:sfpbalance}
\end{figure}

Figure~\ref{fig:sfpbalance} plots both sides of \eqref{eq:sfpbalance} for each structure; the black outline (derivative asset) traces the top of the stacked OCI-plus-retained-earnings area, and Structure~B's stacked area visibly flattens at the FX leg's own maturity, the same mechanism documented in Section~\ref{sec:freezeresult}. Figure~\ref{fig:sankey} restates the identity for Structure~A at maturity as a single-step flow diagram: the terminal derivative asset of KRW 12.79 billion splits into an OCI reserve of KRW 24.41 billion and a cumulative retained-earnings position of $-$KRW 11.62 billion, which sum, with sign, back to the derivative asset.
