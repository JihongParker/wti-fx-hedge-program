...
One convention is fixed here for the whole paper. Structure~A's premium of KRW 34{,}264{,}941{,}589 is the American-exercise LSMC valuation of the combined quanto knock-out, KRW 17{,}132.47 per barrel on 2{,}000{,}000 barrels, taken from the companion pricing paper. The engine walks the accounting ledger on the same American surface, so where Section~\ref{sec:structA} describes the instrument as European-style it is describing the payoff form and not the exercise convention used in valuation.
\label{sec:inception}
...
% =============================================================================
\subsection{What the floor and the clip do to the reported numbers}
...
\end{equation}
The domestic risk-free rate appears nowhere in the final expression: it enters once as $S_2$'s $\mathbb Q$-drift and again, with opposite sign, as the discount rate, and the two cancel identically regardless of the rate's level. What survives is a term $\rho\sigma_1\sigma_2\tau$, and the paper's earlier reading of it requires correction. It is tempting to treat that term as evidence that joint simulation on correlated paths reproduces the quanto drift adjustment automatically. It is in fact the trace of that adjustment being \emph{absent}. Since the dynamics of Section~\ref{sec:sde} give $S_2$ the drift $r_{KRW}-r_{US}$, the simulation measure is $\mathbb Q^{KRW}$; under it $S_1S_2$ is a KRW-denominated traded asset and must drift at $r_{KRW}$, which forces
\begin{equation}
\mathrm{drift}(S_1) \;=\; r_{KRW}-(r_{KRW}-r_{US})-\rho\sigma_1\sigma_2 \;=\; r_{US}-\rho\sigma_1\sigma_2 .
\label{eq:driftcorrect}
\end{equation}
The engine uses $r_{US}$ without the $-\rho\sigma_1\sigma_2$ term, and \eqref{eq:er1r2} carries exactly that omission forward. Correlated shocks fix the joint distribution's shape, not its location; a drift adjustment is an artifact of the change of measure and has to be imposed rather than inherited.
...
\end{equation}
with no correlation term at all. Numerically the correction is $e^{\rho\sigma_1\sigma_2\tau}-1=0.22\%$ of $H^\star$. Its effect on the reported results has been measured rather than assumed: re-running the validated re-implementation on $200{,}000$ paths with the correlation factor removed from $H^\star$, as \eqref{eq:driftcorrect} requires, moves Structure~A's mean absolute ineffectiveness by $+0.76\%$. No ordering, sign, or headline comparison in this paper changes. The correction matters for what the appendix is entitled to claim: \eqref{eq:er1r2} validates the algebra of the joint expectation, and it does not validate the measure specification. Section~\ref{sec:quanto}'s statement that joint simulation reproduces the quanto adjustment automatically is withdrawn. This is also what makes $H^\star$ fit for purpose under IFRS~9.6.3.4: the hypothetical derivative must benchmark the aggregated exposure's multiplicative WTI-times-FX risk linearly and without hedge-specific optionality, and a discounted $\mathbb Q$-expectation of the raw physical settlement is, by construction, a $\mathbb Q$-martingale carrying no barrier or other option-specific feature: the property Section~\ref{sec:hd} requires of it, derived here and not assumed.
