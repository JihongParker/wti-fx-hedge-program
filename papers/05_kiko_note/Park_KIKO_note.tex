...
This note asks a narrow question: \emph{what does a modern hedging research program, built for a different firm and a different risk, have to say about KIKO?} The program in question comprises four companion papers on a Korean crude-oil importer's joint WTI--currency exposure: a budget-constrained allocation framework with a stress ledger and a barrier survival haircut (Park, 2026a); a barrier-option pricing and delta-hedging study built on first-passage Monte Carlo (Park, 2026c); an IFRS~9 cash-flow-hedge accounting engine that measures what designation choices do to the books when barrier events strike (Park, 2026b); and an equilibrium model of disclosure and hedging in which disclosure operates through the price of residual risk (Park, 2026d). None of these papers models KIKO. The claim of this note is that each of them isolates, with numbers, one of the four layers on which KIKO's damage depended -- and that the composite lesson is sharper than any single-layer account.
...
The disclosure paper's equilibrium delivers one sharp policy result: mandates move hedging behavior only through the margin that quantifies the priced exposure -- floors on orthogonal content produce an exact zero, a prediction the paper verifies on two realized Korean mandates. Run backwards over KIKO, the result sorts post-crisis regulatory instincts into two bins. Checkbox disclosure -- ``does the firm use derivatives?'', governance attestations, qualitative risk statements -- sits on the orthogonal margin; the model predicts, and the paper's nulls illustrate, that it changes nothing. The quantity that would have surfaced KIKO before it detonated is a single ratio: \emph{derivative notional to underlying flow}, per currency, marked against its barriers. Firms carrying $2.5\times$ overhedges with knock-ins seven percent away were, in the model's language, carrying an enormous unpriced $R$ that quantified disclosure would have moved directly into $\Lambda$ -- the price their banks, boards, and auditors attached to residual risk -- years before the barrier was touched. The KIKO episode is thus the disclosure paper's exposure-quantification ladder read as history: the rungs that existed in 2007 were orthogonal, and the rung that mattered did not exist.
...
\bibitem{park2026d} Park, J. (2026d). ``Does Mandatory ESG Disclosure Change Corporate Hedging? Theory, and Evidence from Korea's Staggered Disclosure Mandates.'' Working paper, Pusan National University.
