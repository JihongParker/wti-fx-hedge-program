...
Sections~\ref{sec:results}---\ref{sec:sensitivity} expose a tension: the regression delta is \emph{pointwise} noisy (Table~\ref{tab:deltacheck}) yet narrows hedge-cost dispersion on a like-for-like ledger (Section~\ref{sec:decomp}). The resolution is that pointwise accuracy and hedging performance are different quantities: a proxy delta is correct \emph{for a different option}, one with no barrier, and is therefore systematically wrong in a barrier-correlated direction on the large fraction of paths that approach the KO boundary, while the regression delta's error, though large and systematic in level (Section~\ref{sec:awareness}), is not correlated with barrier proximity in the same way. A hedge-cost aggregate, summed and squared over the option's life, punishes systematic bias far more than noise.
...
\emph{The delta-level comparison.} Table~\ref{tab:deltacheck} is measured on the exact engine over five seeds with the contract strike held fixed while the spot is bumped. Getting that wrong, by letting $K$ follow $S_1(0)$, produces a spuriously small sensitivity and an apparent order-of-magnitude discrepancy; an earlier version of this paper reported exactly that artifact and it has been withdrawn. On the correct measurement the gap runs from $7.8\%$ at inception to $72.4\%$ near expiry. The regression side carries a seed dispersion of $0.077$ at inception and should not be quoted as a point estimate at any single seed.
