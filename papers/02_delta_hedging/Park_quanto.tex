...
This is the paper's stated central empirical exercise (Section~\ref{sec:intro}) and its sign does not survive. What survives is the diagnosis in Section~\ref{sec:awareness}: the fitted surface is close to a constant over its fitted domain and does not reproduce the sign reversal a barrier delta carries, so there was never a mechanism by which it should have beaten a well-specified closed form. The earlier result was the artifact; this is the measurement.
...
Equation~\eqref{eq:hcaffine} is an identity forced by the engine: a third run at $c=0.5$ reproduces $A_p+0.5\,B_p$ path by path with maximum relative error below $10^{-15}$. That part of Section~\ref{sec:fxsweep}--\ref{sec:cstar} is structural and survives the correction above; what does not survive is the economic reading placed on the minimizer's location.

An earlier re-implementation of this sweep at $n=200{,}000$ jump-adapted exact paths reported $c^\star=-0.548$ on a ledger that ran to maturity and did not net the oil leg's currency exposure, and read the negative sign as evidence that the two legs' hedge costs co-move so a positively-coupled currency leg adds variance. Equation~\eqref{eq:cstarnet} withdraws that reading. The sign is not information about $\mathrm{Cov}(A,B)$; it is the ledger subtracting a position it should never have opened, and the residual after subtraction is not distinguishable from zero. The quantitative differences between $-0.548$ and $-1.117$ are differences of ledger configuration (exercise-consistent or not, and at what path count), and both are one unit away from a corrected minimizer near zero.

The conclusion this leaves is stronger than the one it replaces, and it is the same conclusion Section~\ref{sec:structdelta} reaches analytically: the currency coupling is not a lever on hedge-cost risk. The oil leg dominates $\mathrm{Var}[A]$, no choice of $c$ can touch that part, and once the book is netted correctly the best available $c$ is nothing.
