...
The gap is largest at inception ($18.6\%$) and shrinks monotonically to $5.6\%$ by $t/T=0.9$; all three points exceed the model's internal $5\%$ reliability flag.

Two explanations compete for the inception figure, and the paper's own convergence evidence selects between them. The first is structural: the step-0 surface must fit curvature across every outcome still live over the full horizon, while near maturity the surviving ITM population, though sparser (Figure \ref{fig:itmdecay}), is more homogeneous and closer to its intrinsic asymptote, an easier local fit. The second is sampling: inception is precisely where the regression delta is least well determined at a given path count. Table~\ref{tab:convpaths} settles it. On the exact engine the inception regression delta reads $0.692$ at $10{,}000$ paths and settles to $0.543$--$0.545$ from $50{,}000$ paths onward, so the $0.7921$ of Table~\ref{tab:deltacheck}, computed on the production bank, sits well outside the converged range while the finite-difference value $0.6679$ is far closer to it. The inception gap is therefore predominantly a small-sample artifact of the production configuration and not evidence about the basis. The monotone shrinkage across $t/T$ is consistent with both stories, since the ITM population that determines sampling precision also becomes more homogeneous with elapsed time; the structural reading survives as a description of that shrinkage, but it should not be carried over to the inception number. Section~\ref{sec:limitations} records the consequence: the reliability flag as reported is triggered partly by sample size, and a converged re-run of this table is the correct object for judging basis adequacy.
...
\midrule
1.00 & 6.790 & 17{,}188.10 & 52{,}507.01 \\
...
\emph{The clipped delta.} The regression delta is clipped to $[0,1]$, which removes the negative deltas an up-and-out call genuinely exhibits near the upper barrier (Section~\ref{sec:delta}). The clip therefore blunts the estimator in exactly the region the paper's barrier-awareness claim rests on, and it biases the comparison in Section~\ref{sec:hedgecost} \emph{against} the regression delta rather than in its favor. An unclipped re-run is not reported here and is the most useful single extension.
...
\emph{The inception delta.} Table~\ref{tab:deltacheck}'s $18.6\%$ finite-difference gap is measured on the production bank, where Table~\ref{tab:convpaths} shows the inception regression delta has not converged. That figure is therefore not evidence about basis adequacy, and Section~\ref{sec:fdcheck} has been read accordingly.
