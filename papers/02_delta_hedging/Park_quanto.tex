...
\bottomrule
\end{tabular}
\caption{The coupling minimizer before and after netting the oil futures leg's own currency exposure. Hedge-cost standard deviation, KRW billion, $60{,}000$ paths, exercise-consistent ledger. The netted column at $c$ equals the gross column at $c+1$ throughout, and the attainable minimum is identical.}
\label{tab:netshift}
\end{table}
...
Equation~\eqref{eq:hcaffine} is an identity forced by the engine: a third run at $c=0.5$ reproduces $A_p+0.5\,B_p$ path by path with maximum relative error below $10^{-15}$. The identity is structural; the economic content sits entirely in where the vertex lands, and on a gross ledger the vertex is the netting artifact. Its location moves with ledger configuration, $-0.548$ on a to-maturity ledger at $200{,}000$ exact paths (Section~\ref{sec:rigor200k}) against the $-1.117$ of Eq.~\eqref{eq:cstarnet}, but every gross configuration sits one unit from a netted minimizer that is economically zero, because the unit is the accidental position itself and not a covariance.\footnote{An earlier version of this paper reported the gross-ledger $c^\star=-0.548$ as a covariance-aware hedge ratio and read its sign as evidence that a positively-coupled currency leg adds co-moving cost variance. Equation~\eqref{eq:cstarnet} is the correction: the sign carries no information about $\mathrm{Cov}(A,B)$.}
...
This also removes an apparent conflict with the companion allocation paper. Park (2026a) finds the currency leg optimally carries a few percent of coverage, $w_2=+0.034$, a small long position; this paper's uncorrected $c^\star$ was a short one, and the two were reported as disagreeing in sign from separate objectives. With the netting correction the dynamic result is $c^\star\approx0$ rather than short, and the two are simply the same finding in two settings: the currency leg is second-order, and what it needs is its own small exposure and not a multiple of the oil leg's.
...
\bibitem{park2026a}
Park, J. (2026a). ``Optimal WTI---FX Hedge Ratios Under a Fixed Budget.'' Working paper, Pusan National University.
