...
\title{\textbf{The Currency Leg of a Knock-Out Oil Option under Jump-Diffusion:\\ Factorization, Netting, and What a Fitted LSMC Delta Does Not Buy}}
...
\begin{abstract}
A Korean crude-oil importer hedging joint WTI and USD/KRW exposure with a barrier oil option settled in won must decide how to size the currency leg. A change of numeraire shows that this payoff factorizes into the spot rate times a pure dollar option, so the correlation desks associate with such structures enters neither its price nor either of its deltas, and the structure's entire currency exposure is its own carrying value per unit of the exchange rate. That is the paper's positive result and it is exact.

The rest is a set of negative results about the estimator such structures are usually hedged with, each obtained by controlling something the original comparison did not. A fitted least-squares Monte Carlo surface delta, examined across its own fitted domain, is close to a constant: it spans $0.033$ where a Black-76 delta spans $0.36$, it never reproduces the sign reversal an up-and-out call carries near knock-out, and it is rank-deficient at inception. Its apparent hedging advantage over a barrier-blind delta is reproduced in full by a constant hedge ratio with no fitting and no barrier information, so the advantage belongs to the level of the ratio and not to barrier awareness. Once the ledger stops where the American price says the contract stops, rather than running to maturity against an American premium, the comparison reverses outright and the closed-form delta is the better hedge. And the variance-minimizing coupling multiplier the coupled family defines is not a covariance-aware hedge ratio but a bookkeeping correction: netting the currency exposure that the oil futures leg already carries moves the minimizer by exactly one unit and leaves nothing economically distinguishable from zero.

The practical recommendation is unchanged and is now reached more directly: size the currency leg to the option's own carrying value, net of what the oil leg already carries, and test any claim that a fitted surface delivers barrier-aware hedging against a constant-delta control before accepting it.
\end{abstract}
...
The structural difficulty is this: the KO feature destroys the applicability of the textbook closed-form models (Black-76 for the European vanilla proxy, Turnbull---Wakeman for the Asian-averaging proxy) that a practitioner would otherwise reach for first, because neither model has a barrier term in its analytic derivation. A Longstaff---Schwartz least-squares Monte Carlo (LSMC) regression, by contrast, prices and differentiates the option \emph{on the same simulated paths that determine the KO event}, so the barrier is handled within the same numerical machinery that produces the hedge ratio. This paper's central empirical exercise is to test that presumption rather than assume it. On an identical common-random-number path bank, so that all three delta-estimation methods see the same realized KO/no-KO outcome, the raw comparison indeed favors the regression surface by an order of magnitude. The paper then adds one control at a time and the advantage does not survive the controls: holding contract, premium and stopping rule fixed shrinks it to $12\%$ (Section~\ref{sec:decomp}); a constant hedge ratio with no barrier information reproduces what remains; and stopping the ledger where the American price says the contract stops reverses the sign outright, leaving the barrier-blind proxy the better hedge on this structure (Section~\ref{sec:consistent}). The measured result is a caution about the estimator, not a vindication of it.
...
\end{equation}
in which neither $\rho$ nor $\sigma_2$ appears. The same holds for any optimal-stopping payoff of this form, since continuation and exercise values are both linear in $S_2$.
\end{proposition}

\noindent The proof is the numeraire change itself: $V(0)=N(0)\,\mathbb E^{\mathbb Q^{US}}[X(T)S_2(T)/N(T)] = S_2(0)e^{-r_{US}T}\mathbb E^{\mathbb Q^{US}}[X(T)]$. Setting $X(T)=\max(S_1(T)-K,0)\mathbf 1\{\text{no barrier touch}\}$ gives this paper's European structure. The American value is a supremum over stopping times and is not of the form \eqref{eq:factorization} covers; the argument extends to it because the numeraire ratio $S_2(t)e^{r_{US}t}/N(t)$ is identically one at every stopping time, so the same change of measure applies pathwise inside the supremum and the exercise region, being $S_1$-measurable, is unchanged. Appendix~\ref{app:amproof} proves the extension. The correlation adjustment $-\rho\sigma_1\sigma_2$ that a KRW-numeraire treatment applies to the drift of $S_1$ is exactly offset by the $+\rho\sigma_1\sigma_2$ generated by the covariance between $X(T)$ and $S_2(T)$; the two cancel, which is what \eqref{eq:factorization} records. Direct simulation confirms it, and confirms it with the barrier in place rather than only for the vanilla case: repricing the double-knock-out payoff at $\rho\in\{-0.5,\,0,\,0.0876,\,0.5\}$ under the correctly-specified measure moves the value by at most $0.6\%$, non-monotonically in $\rho$ and so within Monte Carlo error, against a factorized closed form agreeing to the same tolerance. The survival probability is likewise flat across $\rho$ ($0.741$--$0.743$). This is what \eqref{eq:factorization} predicts: the numeraire change shifts the drift of $S_1$ from $r_{US}-\rho\sigma_1\sigma_2$ under $\mathbb Q^{KRW}$ to $r_{US}$ under $\mathbb Q^{US}$, so the first-passage law that governs the barrier is itself $\rho$-free.

The numeraire argument itself is standard; what it settles here is not. Practitioner usage prices this structure as a quanto and attributes a correlation coupling to it, and the Shapley split of Section~\ref{sec:baseline} allocates premium between a WTI and an FX leg on that basis. Proposition~\ref{prop:factor} shows the coupling is absent from the price, which is a statement about a live pricing convention rather than about the mathematics. Three consequences follow for the rest of the paper. First, the label ``quanto'' is used here in its loose desk sense, for a two-asset structure whose payoff is denominated across currencies, and not in the technical sense of a fixed-conversion-rate contract carrying a correlation-dependent drift adjustment: this structure carries no such adjustment. A genuine quanto would convert at a rate fixed at inception, and only then would $\rho$ survive into the price; in the cross-currency taxonomy the structure studied here is a \emph{composite} (compo) option, and Proposition~\ref{prop:factor} is the textbook composite-option factorization (Musiela and Rutkowski, 2005), restated and verified because the desk convention this paper measures prices the structure as the quanto it is not. Second, Proposition~\ref{prop:factor} \emph{is} the homogeneity result that Section~\ref{sec:structdelta} derives independently and uses to obtain $\delta_{FX}=V/S_2$; the FX-delta results of this paper are therefore consequences of the factorization rather than of any correlation coupling, and they are strengthened by it, since \eqref{eq:factorization} makes the structure's entire FX exposure its own carrying value by construction. Third, the factorization closes the pricing channel but not the question of how the two legs' hedge costs co-move, which Section~\ref{sec:cstar} takes up directly and finds to be dominated by a bookkeeping artifact rather than by the structural correlation.
...
\subsection{Asymmetric Up/Down Jump Decomposition}
\label{sec:emcalib}

The production calibration resolves this by classifying jump days with a robust $k$-sigma threshold and estimating an \emph{up}-jump and a \emph{down}-jump regime separately by moment-matching within each classified regime, since energy-price shocks are empirically asymmetric, with downside dislocations sharper than upside recoveries; the diffusive $\sigma_1$ is then re-estimated from the surviving non-jump days, so the observed quadratic variation is split between the two components rather than duplicated. Appendix~\ref{app:em} states the estimator in full, including what it is not.
...
A note on what this estimator is. Jump days are identified by a $k$-sigma threshold rule and the intensity is their annualised count: $35\times252/1300=6.7846$ per year reproduces the figure the engines use to seven digits, and the two regime intensities are $12\times252/1300$ and $23\times252/1300$. It is a threshold classifier followed by moment-matching within each regime, not the likelihood-maximising mixture that ``EM'' would imply, and the symmetrized triple $(\lambda,\theta_J,\delta_J)=(6.7846,-0.0299,0.08443)$ is what every engine in this paper consumes. Re-deriving the symmetrization from the rounded regime values displayed above returns $\delta_J=0.0834$ rather than $0.08443$, a $1.2\%$ difference that is an artifact of the rounding rather than a second estimate. The estimation sample is the 1{,}300 paired returns of the price history.
...
\begin{tabular}{llll}
\toprule
KO rate & Path bank / measure & Definition & Section \\
\midrule
$0.4369$ & shared bank, $\mathbb Q$ & barrier touch, daily grid & \ref{sec:baseline} \\
$0.2309$ & shared bank, $\mathbb Q$ & barrier touch net of early exercise (American) & \ref{sec:baseline} \\
$0.4551$ / $0.2249$ & FX-sweep bank, $\mathbb Q$ & exercise / KO net of exercise & \ref{sec:fxsweep} \\
$0.4391$ / $0.4377$ & rigor bank, $\mathbb Q$ & barrier touch, exact / production scheme & \ref{sec:rigor} \\
$0.4512$ / $0.4494$ & rigor bank, $\mathbb P$ & barrier touch, exact / production scheme & \ref{sec:rigor} \\
$0.171$--$0.174$ & $k$-sweep banks, $\mathbb Q$ & KO net of exercise, re-fitted and re-struck per $k$ & \ref{sec:lambdarob} \\
\bottomrule
\end{tabular}
\caption{Reference table of every knock-out rate reported in this paper, with the path bank and definition each figure belongs to. Rates from different banks or definitions are not comparable with one another; each section's inference uses only rates from its own row.}
\label{tab:koref}
\end{table}
...
On the clean subset, though see Section~\ref{sec:decomp} for why this control is insufficient,, the LSMC regression delta's mean hedge cost is KRW $64.6$ billion lower and its standard deviation is roughly $2.5\times$ smaller than the Black-76 proxy delta's, on paths where both models are pricing and differentiating literally the same closed-form payoff. This does not isolate the delta rule. Conditioning on the American engine's own exercise decision is selection on an endogenous, delta-correlated event, and a proper control that fixes contract, premium and stopping time leaves a factor of $1.12$ rather than $15.8$. What remains true of the clean subset is only that one delta is computed on the same simulated paths that determine the P\&L, and the other from a model that has no barrier term at all. As a final decomposition, the raw option-payoff bias between engines on identical paths (independent of hedge cost) is $\text{Mean}(V_{Am})-\text{Mean}(V_{Eu})=-\text{KRW }23.86\text{ billion}$ and $\text{Mean}(V_{Am})-\text{Mean}(V_{As})=-\text{KRW }28.57\text{ billion}$, confirming that early exercise materially lowers American's realized option payoff relative to the European and Asian proxies on the same underlying paths, separately from and in addition to its hedge-cost advantage.

\subsection{Decomposing the Gap: The Contribution of the Delta Rule}
\label{sec:decomp}
...
The constant is not an optimum and must not be presented as one. Dispersion falls monotonically as the ratio falls across the whole tested range, from KRW $69.6$bn at $\delta=1.00$ through $57.0$ at $0.80$, $51.0$ at $0.70$, $49.8$ at $0.68$, $45.1$ at $0.60$ to $39.5$ at $0.50$, with no interior minimum. The value $0.70$ was chosen because it is the level at which a constant reproduces the fitted surface, not because it optimises anything, and a constant of $0.50$ beats Black-76 by thirty percent rather than twelve.

This is the paper's most useful result, and it is prescriptive rather than merely negative. On this structure and this calibration, a barrier-aware LSMC regression delta does not beat a barrier-blind closed-form delta because it is barrier-aware. It beats it because Black-76's delta rises toward one as the option goes into the money while a knock-out structure's does not, so any rule that keeps the ratio below Black-76's captures most of the gain. The five-term surface is one such rule; a constant at the surface's own level is another, is cheaper, and is slightly better. Claims that a fitted exercise surface delivers barrier-aware hedging should be tested against a constant-delta control before being accepted.

Even the $12\%$ on this like-for-like ledger is not the end of the decomposition. This ledger runs both arms to maturity, and Section~\ref{sec:consistent} shows that holding the contract fixed \emph{that} way charges a European payoff an American premium; stopping the ledger where the price says the contract stops reverses the sign, and the barrier-blind delta becomes the better hedge. The $12\%$ should therefore be read as the largest advantage the surface retains under any consistent control, not as a standing result: the much larger raw gap in Table~\ref{tab:hedgecost} is dominated by payoff, premium and stopping-time differences between the three engines rather than by delta quality, and what is left once those are removed does not survive the exercise-consistency correction either. The trimodality of Section~\ref{sec:pldist} is a property of the proxy ledgers as configured and inherits the same caveat.

\subsection{The comparison under a consistent ledger}
\label{sec:consistent}

Section~\ref{sec:decomp} holds the contract fixed by running both arms to maturity. That is one of two ways to make the comparison consistent, and it is the wrong one: the premium charged in every ledger is $\mathrm{fit}[\mathrm{price}]$, an American value produced by a backward induction that applies the exercise rule, so a ledger that runs to maturity charges an American premium against a European payoff. The consistent alternative stops the ledger at the fitted exercise time and books intrinsic value there.

Under that correction the central comparison of this paper reverses.

\begin{center}
\small
\begin{tabular}{lrrl}
\toprule
ledger & surface $\delta$ & Black-76 $\delta$ & winner \\
\midrule
as shipped (no exercise, gross FX) & $51.50$bn & $57.42$bn & surface, by $1.12\times$ \\
exercise-consistent & $23.81$bn & $12.58$bn & Black-76, by $1.89\times$ \\
exercise-consistent, FX netted & $20.53$bn & $12.58$bn & Black-76, by $1.63\times$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Hedge-cost standard deviation, $60{,}000$ paths on a surface fitted at $200{,}000$. The regression delta beats the closed-form proxy only in the configuration that charges a European ledger an American premium. Once the ledger stops where the price says the contract stops, the barrier-blind Black-76 delta is the better hedge on this structure by a factor between $1.6$ and $1.9$, and netting the oil leg's currency exposure widens the gap rather than closing it.

This completes the decomposition the Introduction promised. What survives of the raw comparison is the diagnosis in Section~\ref{sec:awareness}: the fitted surface is close to a constant over its fitted domain and does not reproduce the sign reversal a barrier delta carries, so there was never a mechanism by which it should have beaten a well-specified closed form. The configuration in which it appears to win is the one that mixes two contracts.
...
\subsection{State Dependence of the Fitted Surface}
\label{sec:awareness}
...
The summary is therefore specific. The five-term surface is not a barrier-aware delta estimator in the sense the term usually implies: over the fitted domain it is close to a constant, it does not reproduce the barrier's characteristic sign reversal, and it is poorly conditioned at inception. That is a substantive finding about a widely used construction, not a defect peculiar to this implementation, and it is measurable only because the structure admits an independent price sensitivity to test against. What Section~\ref{sec:hedgecost} measures is the performance of the \emph{hedging rule} the surface induces, which is a different and still useful question.
...
Two qualifications are essential to reading $c^\star$, and the second is decisive.

The first concerns what is being minimized. In Ederington's setting $A$ is the firm's \emph{spot exposure}. Here $A=\mathrm{HC}_p(0)$ is the WTI leg's own hedge cost, so \eqref{eq:hcvar} is the variance of the \emph{hedging programme's ledger} and not of the firm's total position: the importer's physical exposure, $\$157{,}880{,}000$ of monthly dollar settlement against $2{,}000{,}000$ barrels, appears in neither $A$ nor $B$. A treasury minimizing total-P\&L variance, with the physical exposure written into $A$, would obtain a different multiplier.

The second is that $c^\star$ is not measuring a covariance at all in the ledger that produced it. The WTI hedge is carried as $g_1$ barrels of futures whose value is $g_1S_1S_2$, so \emph{the oil leg already holds $g_1S_1$ dollars}. A currency leg set to $\delta_{FX}$ on top of that leaves the book with a total exchange-rate sensitivity of $g_1S_1+\delta_{FX}$, not $\delta_{FX}$. The production ledger never subtracts the first term. Measured on the fitted surface, the unintended position averages roughly four times the structural requirement \eqref{eq:structdelta}, and the variance minimizer spends itself cancelling it.

The test is decisive. Re-running the sweep with the oil leg's own currency exposure netted out, so that the book's total sensitivity is what $\delta_{FX}$ says it is, moves the vertex by
\begin{equation}
c^\star_{\text{gross}} = -1.117 \;\longrightarrow\; c^\star_{\text{net}} = -0.117,
\label{eq:cstarnet}
\end{equation}
a shift of exactly $1.000$, and leaves the attainable hedge-cost standard deviation unchanged at KRW $20.32$bn. The unit shift is the algebraic signature of the mechanism rather than a coincidence: netting removes precisely one unit of the coupled position, because the oil leg's currency exposure \emph{is} the $c=1$ position. The two ledgers are the same book relabeled, which Table~\ref{tab:netshift} shows directly.
...
\begin{tabular}{lrrrr}
\toprule
ledger & $c^\star$ & sd at $c=0$ & sd at $c=1$ & sd at $c^\star$ \\
\midrule
gross (as shipped) & $-1.117$ & $22.80$ & $28.25$ & $20.32$ \\
oil-leg FX netted  & $-0.117$ & $20.34$ & $22.80$ & $20.32$ \\
\bottomrule
\end{tabular}
\caption{The coupling minimizer before and after netting the oil futures leg's own currency exposure. Hedge-cost standard deviation, KRW billion, $60{,}000$ paths, exercise-consistent ledger. The gross column at $c$ equals the netted column at $c+1$ throughout, and the attainable minimum is identical.}
\label{tab:netshift}
\end{table}

\noindent One full unit of the reported multiplier is therefore a bookkeeping correction. What survives once the accidental position is gone is $c^\star=-0.117$, and the honest measure of what the optimization buys is its gain over simply not coupling at all: from KRW $20.34$bn at $c=0$ to KRW $20.32$bn at $c^\star$, a reduction of $0.14\%$. The $28\%$ that the same optimization appears to deliver on the gross ledger is, to within a seventh of a percent, the cost of the position the ledger created by accident. An independent adjudication engine that never books the futures leg's currency term at all returns $c^\star\approx0$ on the same structure, which is the same result reached from the opposite construction.

Section~\ref{sec:structdelta}'s $V/S_2$ is unaffected, since it is derived analytically rather than from the ledger. The consequence for this paper is that the coupled-family apparatus of Sections~\ref{sec:fxsweep}--\ref{sec:cstar} should be read as diagnosing a netting error rather than as recovering a covariance-aware hedge ratio, and the recommendation it supports, size the currency leg to $V/S_2$, is reached more directly and more robustly from Proposition~\ref{prop:factor} alone.

Equation~\eqref{eq:hcaffine} is an identity forced by the engine: a third run at $c=0.5$ reproduces $A_p+0.5\,B_p$ path by path with maximum relative error below $10^{-15}$. The identity is structural; the economic content sits entirely in where the vertex lands, and on a gross ledger the vertex is the netting artifact. Its location moves with ledger configuration, $-0.548$ on a to-maturity ledger at $200{,}000$ exact paths (Section~\ref{sec:rigor200k}) against the $-1.117$ of Eq.~\eqref{eq:cstarnet}, but every gross configuration sits one unit from a netted minimizer that is economically zero, because the unit is the accidental position itself and not a covariance.

The conclusion is the one Section~\ref{sec:structdelta} reaches analytically: the currency coupling is not a lever on hedge-cost risk. The oil leg dominates $\mathrm{Var}[A]$, no choice of $c$ can touch that part, and once the book is netted correctly the best available $c$ is nothing.

With the netting correction the dynamic result is $c^\star\approx0$: what the currency leg needs is its own small exposure and not a multiple of the oil leg's.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.78\textwidth]{figures/fig_cstar_parabola.pdf}
\caption{Hedge-cost standard deviation as the exact parabola $\sqrt{\mathrm{Var}[\mathrm{HC}(c)]}$ implied by the affine decomposition~\eqref{eq:hcaffine}. The parabola itself is an identity; its vertex is not the covariance-aware multiplier the figure was originally drawn to display, and lies one unit from zero for the reason Eq.~\eqref{eq:cstarnet} gives.}
\label{fig:cstar}
\end{figure}
...
\end{equation}
exactly linear in $S_2(t)$: the $S_2$ factor pulls out of the expectation as $S_2(t)e^{(r_{KRW}-r_{US})\tau}$ while the tilt shifts $S_1$'s drift under $\mathbb Q^{KRW}$ by the term $\rho\sigma_1\sigma_2$, which Proposition~\ref{prop:factor} shows is exactly what the numeraire change removes, and leaves the barrier, the jumps, and (since both continuation and exercise values are linear in $S_2$) the early-exercise region untouched. Homogeneity of degree one in $S_2$ gives the structural FX delta in closed form,
\begin{equation}
...
Two internal cross-checks accompany this. First, the fitted regression surface provides a second, independent estimate of the same delta, $\partial\tilde V/\partial S_2 = (\beta_2+2\beta_4 v_2+\beta_5 v_1)/S_2(0)$, computed from the very coefficients that price the option (Section~\ref{sec:delta}); it agrees with the homogeneity form $V/S_2$ to a median $8.8\%$ over the surface, so the fitted basis respects the structure's known $S_2$-linearity to within that gap and the residual is a measurable property of the five-term polynomial and not of the theorem, which is exact. Second, the structural delta~\eqref{eq:structdelta} is \emph{positive}, since the option's KRW value rises with the exchange rate, whereas the variance-minimizing multiplier of Section~\ref{sec:cstar} came out \emph{negative}. The two do not disagree. Equation~\eqref{eq:cstarnet} shows the multiplier was negative because it was subtracting a currency position the ledger had created by accident, and once that position is netted the multiplier is essentially zero and the two point the same way. That is what the theory predicts. If $V/S_2$ is already the right currency exposure, then a correctly-netted book needs no further coupled adjustment, and the minimizer should sit at zero. It does.
...
On the exact engine at $200{,}000$ paths, the affine identity~\eqref{eq:hcaffine} holds free of the discretization term the production scheme carries, hedge-cost standard deviation at $c=1$ is KRW $51.90$bn, and the vertex sits at $c^\star=-0.548$ with $48.76$bn. That vertex is the gross-ledger quantity of Section~\ref{sec:cstar}: exact simulation removes discretization error, not the netting error, and the two are independent. Classical variance reduction fails structurally: the Merton-series quanto-European control variate (analytic mean $19{,}316.55$ vs.\ simulated $19{,}314.12$) gains only $\times1.01$, since KO'd paths zero the option payoff where the control is largest; antithetic pairing gains $\times1.00$, the payoff being non-monotone in the shocks. On this product, accuracy has to come from path count and not from variance reduction.

\section{Discussion}
\label{sec:discussion}

Sections~\ref{sec:results}---\ref{sec:sensitivity} expose a tension: the regression delta is \emph{pointwise} noisy (Table~\ref{tab:deltacheck}) yet narrows hedge-cost dispersion on a like-for-like ledger (Section~\ref{sec:decomp}). The resolution is that pointwise accuracy and hedging performance are different quantities: a proxy delta is correct \emph{for a different option}, one with no barrier. Section~\ref{sec:decomp} identifies what the aggregate actually punishes, and it is not barrier-correlated error: a constant hedge ratio carries no barrier information at all, its error is largest near the boundary, and it nonetheless reproduces the fitted surface's entire advantage. What the aggregate punishes is the \emph{level}. Black-76's delta rises toward one as the option goes into the money while a knock-out structure's does not, so a rule that keeps the ratio below Black-76's captures most of the available reduction, whether it obtains that ratio by fitting or by assumption.
...
What the closed-form results of Sections~\ref{sec:cstar}---\ref{sec:structdelta} add is that the coupled family is the wrong family, and for a reason that has nothing to do with covariance. A rule of the form $\delta_{FX}=c\,\delta_{WTI}$ sizes the currency leg off the WTI hedge notional, whereas the exposure that needs hedging is the option's own carrying value $V/S_2$: at $c=1$ the coupling books a median $4.6\times$ what the structure requires. And the multiplier that appeared to correct this within the family was correcting something else. Once the oil futures leg's own currency exposure is netted, the variance-minimizing $c$ falls to $-0.117$ and buys $0.14\%$ over not coupling at all (Eq.~\ref{eq:cstarnet}). There is no covariance-aware coupling to be had on this structure (Section~\ref{sec:cstar}).

The recommendation is correspondingly one-part. Size the currency leg to $V/S_2$, net of whatever currency exposure the oil hedge already carries, and do not tune $c$ at all: the parameter has no economic content on a correctly-netted book, and on a gross book the optimizer's apparent gains are the cost of the accident it is undoing.

For the three audiences that actually run this trade, the findings translate into different operational statements. For the \emph{importer's treasury}, with the caveat of Section~\ref{sec:cstar} that $V/S_2$ is a property of the option book rather than of the physical purchase programme, the headline is that sizing the FX leg as a fixed positive multiple of the WTI delta increases the option book's hedge cost without reducing its risk: every additional unit of positively-coupled FX exposure raised both the mean and the variance of realized hedge cost across the entire tested range, so a one-for-one coupling adds exposure under the label of a hedge and not buying a conservative default with premium, on a leg whose true size (the option's own carrying value, $V/S_2$) is roughly a factor of four to five smaller than what that coupling books. For the \emph{commodity desk}, the finding with teeth is negative: the fitted regression delta does not beat a barrier-blind closed-form delta once the ledger is made exercise-consistent (Section~\ref{sec:consistent}), and where it does appear to win, a constant hedge ratio with no barrier information reproduces the win in full (Section~\ref{sec:decomp}). The operational content is a control rather than a model choice. Before paying for a fitted exercise surface on a knock-out structure, run the constant-delta control and the exercise-consistent ledger; on this structure both of them dissolve the case for the surface. For a \emph{structured-products desk} quoting these notes, the KO-rate decomposition matters commercially: the American engine's reported knock-out statistic understates the raw barrier-touch frequency by roughly half because early exercise pre-empts the tally, so a desk quoting KO probabilities off engine output without that decomposition would systematically misstate the barrier risk its clients are selling. None of these statements requires adopting this paper's specific model; each follows from mechanics that survive any recalibration: a coupling rule, a barrier a model cannot see, and a competing stopping time.

\section{Limitations}
\label{sec:limitations}

Hedge rebalancing is discrete (daily, 217 steps), interest rates are constant, and the jump-intensity sensitivity of Section~\ref{sec:jumpsens} is a partial measure change bounded by the Esscher-consistent tilt of Section~\ref{sec:esscher} at $\approx1.1\%$. Hedge-accounting treatment and static budget allocation are outside the scope. Five boundaries bear on headline numbers and are stated with their measured size.

\emph{Estimation risk in the jump block.} Jumps are rare, so $(\lambda,\theta_J,\delta_J)$ rest on 35 classified days out of 1{,}300 returns, the likelihood surface is flat along the intensity-versus-amplitude direction, and the standard errors are correspondingly wide. Section~\ref{sec:rigor} controls discretization and Monte Carlo error at a \emph{fixed} parameter vector and says nothing about how well that vector is known. Because the knock-out probability depends on the jump tail, plausible variation within the jump block's own confidence region would move the KO rate, the premium and the level of hedge cost by more than every numerical residual catalogued here. Read the hedge-cost levels as central estimates inside a wide input-driven band. Propagating the calibration covariance, or integrating over a jump-parameter posterior, is the natural extension.

\emph{The estimator's level.} Table~\ref{tab:deltacheck} measures the surface gradient against a finite difference taken with the contract strike held fixed while the spot is bumped; the gap runs from $7.8\%$ at inception to $72.4\%$ near expiry, and Section~\ref{sec:awareness} shows the gradient is close to constant over the fitted domain. Comparisons of hedging \emph{rules} on common paths, which is what every headline here is, do not require an accurate $\partial V/\partial S_1$; quoting the gradient as a price sensitivity does, and would be wrong by up to seventy percent late in the option's life. The inception regression is also poorly conditioned, $\mathrm{cond}(X'X)\approx1.8\times10^{10}$, which is why its seed dispersion is $0.077$ and why $\partial V/\partial S_2$ is sign-unstable there.

\emph{Measure consistency of the production scheme.} The engine simulates $S_1$ with drift $r_{US}$, $S_2$ with drift $r_{KRW}-r_{US}$, and discounts the won payoff at $r_{US}$. Proposition~\ref{prop:factor} gives the correctly specified value, against which this combination carries a spurious $e^{(r_{KRW}-r_{US})T}\approx-0.42\%$ and a spurious $\rho\sigma_1\sigma_2$ drift tilt worth about $+1\%$; the net is $+0.4\%$ at the calibrated correlation, below the $1.8\%$ discretization residual, but $+5.3\%$ at $\rho=0.5$. Any use of this engine at a materially different correlation should re-derive the drift from \eqref{eq:factorization}.

\emph{What the hedge-cost comparison can support.} Table~\ref{tab:hedgecost}'s three engines differ in payoff, premium and stopping time as well as in delta rule, so its raw gap is not attributable to delta quality; Section~\ref{sec:decomp} gives the controlled comparison and the constant-ratio control. The trimodality of Section~\ref{sec:pldist} is resolved at a default kernel bandwidth and collapses at wider ones; no dip test is offered.

\emph{Ledger scope.} Every hedge-cost figure here is the variance of the option-hedging ledger, in which the importer's physical dollar purchase appears nowhere. Section~\ref{sec:cstar}'s netting correction is internal to that ledger: it removes the currency exposure the oil futures leg carries against the option, not the exposure of the physical purchase programme. A treasury minimizing total-position variance, with the physical leg written in, faces a different optimization than the one solved here, and its answer for the currency leg would be a genuine positive exposure rather than the $V/S_2$ of the option book alone. The recommendation of this paper is scoped to the derivative book.

Subject to these boundaries, the paper's findings are comparative statements made on a common path bank and a common parameter vector, and are for that reason more robust to input uncertainty than the absolute levels: they turn on the factorization of \eqref{eq:factorization}, on the shape of the fitted surface, and on the structural mismatch between a barrier-free model and a barrier-carrying instrument.

\section{Conclusion}
\label{sec:conclusion}

This paper set out to derive a covariance-aware hedge ratio for the currency leg of a barrier oil option, and found instead that the coupling it was built on is not where the economics sit.

What holds is analytic. A change of numeraire factorizes the payoff into the spot rate times a dollar option, so neither the price nor either delta depends on the correlation between the legs (Proposition~\ref{prop:factor}), and homogeneity then fixes the currency delta at the option's own carrying value $V/S_2$ (Section~\ref{sec:structdelta}). The per-path affine decomposition of hedge cost in the coupling parameter is likewise an identity, reproduced to machine precision, and the knock-out rate partition into exercise, barrier and survival is a measured property of the path bank.

What does not hold is the empirical apparatus built on top. The fitted regression surface is close to a constant over its fitted domain and carries almost none of the state dependence a barrier delta requires (Section~\ref{sec:awareness}); its advantage over a closed-form proxy is reproduced by a constant hedge ratio (Section~\ref{sec:decomp}) and reverses outright once the ledger is made exercise-consistent (Section~\ref{sec:consistent}); and the negative coupling multiplier is a correction for currency exposure the oil futures leg already carries, vanishing to within noise when that exposure is netted (Section~\ref{sec:cstar}).

Two of these are worth stating as prescriptions rather than as findings about one structure. A claim that a fitted exercise surface delivers barrier-aware hedging should be required to beat a constant-delta control before it is accepted. And a hedge ledger that carries a futures leg in a foreign currency must net that leg's own currency exposure before any currency-leg optimization is run, or the optimizer will spend itself undoing an accident.
...
\section{Proof of the American Extension of Proposition~\ref{prop:factor}}
\label{app:amproof}

Let $N(t)=S_2(t)e^{r_{US}t}$, the KRW value of the USD money-market account, a traded KRW asset with $N(0)=S_2(0)$. Its discounted price $Z_t:=N(t)e^{-r_{KRW}t}/N(0)$ is a positive $\mathbb Q^{KRW}$-martingale with $Z_0=1$, and defines the measure change $d\mathbb Q^{US}/d\mathbb Q^{KRW}\big|_{\mathcal F_t}=Z_t$. For any stopping time $\tau\le T$ and any $\mathcal F_\tau$-measurable payoff, optional stopping applied to $Z$ gives the abstract Bayes identity $\mathbb E^{KRW}[Z_\tau Y]=\mathbb E^{US}[Y]$, so
\[
\mathbb E^{KRW}\!\big[e^{-r_{KRW}\tau}X_\tau S_2(\tau)\big]
=\mathbb E^{US}\!\Big[\frac{e^{-r_{KRW}\tau}X_\tau S_2(\tau)}{Z_\tau}\Big]
= S_2(0)\,\mathbb E^{US}\!\big[e^{-r_{US}\tau}X_\tau\big],
\]
since $e^{-r_{KRW}\tau}S_2(\tau)/Z_\tau = S_2(0)e^{-r_{US}\tau}$ identically. Equivalent measures share null sets and hence the same family of $\mathcal F$-stopping times, so taking suprema on both sides gives $V^{\mathrm{Am}}(0)=S_2(0)\sup_\tau\mathbb E^{US}[e^{-r_{US}\tau}X_\tau]$. Finally, the optimand $e^{-r_{US}t}X_t$ is adapted to the sub-filtration $\mathcal F^{S_1}$ generated by the $S_1$ path and its barrier state; conditioning any $\mathcal F$-stopping rule down to $\mathcal F^{S_1}$ by the tower property leaves the value of the $\mathcal F^{S_1}$-Snell envelope unchanged, so the supremum is attained on $\mathcal F^{S_1}$-stopping times and the exercise region is $S_1$-measurable, free of $\rho$ and $\sigma_2$. $\square$

\section{The Jump-Classification and Moment-Matching Construction}
\label{app:em}

Each daily return $r_i=\ln(S_1(t_i)/S_1(t_{i-1}))$ is classified as a jump day when it exceeds a robust $k$-sigma band around a robust location estimate of the return series, at the production threshold $k=3$; classified days split by sign into the up and down regimes. Within each regime the intensity is the annualised count, $\lambda_r=n_r\cdot252/N$; the jump mean $\theta_{J,r}$ and dispersion $\delta_{J,r}$ are the sample moments of the classified returns; and the diffusive $\sigma_1$ is re-estimated from the unclassified remainder, so the observed quadratic variation is split between components and never duplicated, the identification discipline Section~\ref{sec:calibration} requires. At $k=3$ on the $1{,}300$-return sample this yields $n_{up}=12$, $n_{dn}=23$, $\lambda=35\times252/1300=6.7846$, and the regime moments of Table~\ref{tab:asymjump}; Section~\ref{sec:lambdarob} sweeps $k\in\{2.5,3.0,3.5\}$, which measures the estimator's sensitivity to the one tuning constant it has.

For clarity about what the estimator is not: it is a hard classification followed by moment matching, not an expectation-maximization fit. No posterior membership weights are computed and no likelihood is ascended; a genuine Poisson-mixture likelihood would weight every return partially into every component. The hard-threshold construction is used because jump days at these intensities are sparse enough that classification is nearly deterministic, and because the sensitivity that matters, the dependence on $k$, is measurable directly rather than integrated out.
...
\bibitem{musiela2005}
Musiela, M., \& Rutkowski, M. (2005). \textit{Martingale Methods in Financial Modelling} (2nd ed.). Springer.
