...
\midrule
Symmetrized (moment-matched, Eqs.\ above) & 6.7846 & $-2.99\%$ & $8.34\%$ & 35 \\
\bottomrule
\end{tabular}
...
The re-measured comparison is not a small discrepancy and it does not shrink with elapsed time. The regression surface's analytic gradient exceeds the finite-difference sensitivity by a factor of $8.5$ at inception, $8.7$ at mid-life and $20.5$ near expiry. The finite-difference side is the tightly determined one: across five seeds its standard deviation is $0.004$ or less at every point, it is unchanged when the bump is moved from $5\%$ to $1\%$, and it agrees with a seven-point price-versus-spot regression over $S_1(0)\pm10\%$, which returns $0.0635$ at inception. The regression side is both larger and far noisier, with a seed dispersion of $0.077$ at inception.

The direction of the discrepancy identifies its cause. The price-versus-spot curve for this structure rises from $S_1=71$ to a maximum near $S_1=83$ and then \emph{falls}, because raising spot buys intrinsic value and simultaneously buys knock-out probability against the upper barrier at $U=120$. A structure whose value turns over inside the tested range has a small delta at the money, and $0.064$ is that number. The fitted five-term surface reproduces none of this: at inception its gradient is $0.5623$, which coincides to four decimal places with the Black-76 delta of a barrier-free call on the same strike, $0.5623$.

That coincidence is the finding. On the fitted domain, which is in-the-money survivors only, the polynomial basis $\{1,v_1,v_2,v_1^2,v_2^2,v_1v_2\}$ has almost no exposure to paths near extinction, so the surface it fits is close to a barrier-free surface and its gradient inherits a barrier-free delta. The estimator this paper has been calling barrier-aware is barrier-aware in its \emph{state dependence} but not in its \emph{level}: Section~\ref{sec:hedgecost} measures its hedge-cost advantage over the Black-76 proxy at $15.8\times$ in mean on a common bank, and Section~\ref{sec:awareness} below shows the two deltas decouple as the option ages, with a cross-sectional correlation that turns from $+0.85$ at $t/T=0.25$ to $-0.97$ at mid-life, and with the surface delta damped to $0.65$--$0.80$ against Black-76's $0.91$--$1.00$ on paths within ten percent of the upper barrier. The surface therefore does carry barrier information, and that information is what the hedge-cost comparison rewards; what it does not carry is a correct magnitude for $\partial V/\partial S_1$.

Two consequences follow for how this paper's results should be read, and both are recorded in Section~\ref{sec:limitations}. The hedge-cost ranking between estimators is a comparison of two hedging \emph{rules} on identical paths and is unaffected. Any use of the regression gradient as a point estimate of the structure's price sensitivity, however, would be wrong by roughly an order of magnitude, and a desk sizing a hedge from it would over-hedge the WTI leg accordingly.

\subsection{Is the surface barrier-aware? A direct test}
\label{sec:awareness}

Because the level test above is negative, it is worth asking separately whether the surface carries barrier information at all. Table~\ref{tab:aware} compares the surface delta with the Black-76 delta on the same $200{,}000$-path bank at five points in the option's life, both on all hedged nodes and on the subset within ten percent of the upper barrier.
...
\toprule
$t/T$ & hedged nodes & surface & Black-76 & corr. & \multicolumn{1}{c}{surface / B76, $S_1>110$} \\
\midrule
0.00 & 101{,}887 & 0.5623 & 0.5623 & $+0.20$ & --- \\
0.25 &  95{,}192 & 0.6487 & 0.7046 & $+0.85$ & $0.670$ / $0.914$ \\
0.50 &  80{,}001 & 0.7034 & 0.7552 & $-0.97$ & $0.647$ / $0.950$ \\
0.75 &  64{,}969 & 0.7827 & 0.8086 & $-0.93$ & $0.752$ / $0.985$ \\
0.95 &  54{,}523 & 0.9383 & 0.9042 & $-0.74$ & $0.796$ / $0.998$ \\
\bottomrule
\end{tabular}
\caption{Regression-surface delta against the barrier-blind Black-76 delta on a common $200{,}000$-path bank. Near the upper barrier the surface is damped relative to Black-76 and the damping widens with elapsed time, but the surface delta never approaches zero and never reverses sign, which a true up-and-out delta does.}
\label{tab:aware}
\end{table}

The answer is a qualified yes. At inception the two are indistinguishable, so the surface adds nothing. From a quarter of the way through the option's life they separate: the cross-sectional correlation turns negative, meaning the surface orders states differently from Black-76 rather than merely scaling it, and near the upper barrier the surface is damped by twenty to thirty percent relative to the barrier-blind delta, with the damping widening as expiry approaches. What the surface never does is reverse sign. Over $17{,}342{,}543$ hedged nodes the minimum raw delta is $+0.357$, and within ten percent of the upper barrier it is $+0.582$: the negative deltas an up-and-out call genuinely carries near extinction are absent from this estimator entirely.

This settles a question left open in Section~\ref{sec:delta}. The $\mathrm{clip}_{[0,1]}$ applied to the raw delta cannot be what removes those negative values, because the raw delta never produces one. The lower clip is never binding; the upper clip binds on $1.42\%$ of hedged nodes. Re-running the full hedge with the clip removed changes the hedge-cost mean not at all and the standard deviation from KRW $51.25$bn to $51.27$bn, four hundredths of one percent. The limitation is therefore not the clip but the basis: a quadratic in $(v_1,v_2)$ fitted on in-the-money survivors cannot represent a region in which value falls as spot rises, and enriching the basis toward the barrier is the extension that would test whether the level error closes.
